...
void CEledoradoEvent::RegenGoldGoblen()
{
	...
	for (n = 0; n < OBJ_MAXMONSTER; n++)
	{
		if (gObj[n].Class == 78)
		{
			...
			LogAdd("Make GoldGoblen : %d, %d,%d", MapNumber, gObj[n].X, gObj[n].Y);
		}
	}
}
	...
	for (n = 0; n < OBJ_MAXMONSTER; n++)
	{
		if (gObj[n].Class == 79)	// Gold Dercon
		{
			...
			if (SelMap == -1)
			{
				...
				{
				case 0:
					strcat(Buffer, lMsg.Get(MSGGET(7, 208)));
					break;
				case 2:
					strcat(Buffer, lMsg.Get(MSGGET(7, 210)));
					break;
				case 3:
					strcat(Buffer, lMsg.Get(MSGGET(7, 211)));
					break;
				}

				strcat(Buffer, lMsg.Get(MSGGET(4, 134)));

				AllSendServerMsg(Buffer);
			}
...
void CEledoradoEvent::RegenDevilLizardKing()
{
	...
	for (n = 0; n < OBJ_MAXMONSTER; n++)
	{
		if (gObj[n].Class == 80)	// Devil Lizard King
		{
			...
			LogAdd("Make GoldLizarKing : %d, %d,%d",MapNumber, gObj[n].X, gObj[n].Y);
		}
...
void CEledoradoEvent::RegenKantur()
{
	...
	for (n = 0; n < OBJ_MAXMONSTER; n++)
	{
		if (gObj[n].Class == 82)	// Gold Tantalos
		{
			...
			LogAdd("Make GoldTantarus : %d, %d,%d",MapNumber, gObj[n].X, gObj[n].Y);
		}
...
void CEledoradoEvent::End_Menual()
{
	for (int i = 0; i < 3; ++i)
	{
		if (this->m_BossGoldDerconMapNumber[i] != -1)
		{
			GCMapEventStateSend(this->m_BossGoldDerconMapNumber[i], 0, 3);
			this->m_BossGoldDerconMapNumber[i] = -1;
		}
	}
	...
	for (int n = 0; n < OBJ_MAXMONSTER; ++n)
	{
		if ((gObj[n].Class >= 53 && gObj[n].Class <= 54) ||
			(gObj[n].Class >= 78 && gObj[n].Class <= 83))
		{
			gObj[n].Live = FALSE;
			gObj[n].MapNumber = -1;
