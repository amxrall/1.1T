...
bool cDeathEvent::Load()
{
	...
	Token.ParseFile(std::string(".\\Data\\Eventos\\DeathEvent.txt"), Group);
		...
		for (int i = 0; i < this->_Count; i++)
		{
			if (Now.wDayOfWeek == this->DeathStruct[i]._Day && Now.wHour == this->DeathStruct[i]._Hours && Now.wMinute == this->DeathStruct[i]._Minutes && Now.wSecond == 0)
			{
				...
				for (int Index = OBJ_STARTUSERINDEX; Index < OBJMAX; ++Index)
				{
					...
					{
						Function.MsgOutput(Index, 0, "[   DEATH-MATCH   ]");
	...
	if (this->_Progress == true)
	{
		if (lpObj->Type == OBJ_USER && Target->Type == OBJ_USER)
		{
			if (this->PlayerStruct[lpObj->m_Index]._InEvent == true && this->PlayerStruct[Target->m_Index]._InEvent == true)
			{
				...
				Function.MsgOutput(lpObj->m_Index, 1, "+1 Score recebido. Total: %d", this->PlayerStruct[lpObj->m_Index]._Kills);
...
void cDeathEvent::Winner()
{
	...
	if (this->_Value && this->PlayerStruct[this->_Value]._Kills > 0)
	{
		...
		for (int i = OBJ_STARTUSERINDEX; i < OBJMAX; ++i)
		{
			...
			{
				Function.MsgOutput(i, 0, "[   DEATH-MATCH   ]");

				Function.MsgOutput(i, 0, "%s Winner, %d Kills", gObj[this->_Value].Name, this->PlayerStruct[this->_Value]._Kills);
