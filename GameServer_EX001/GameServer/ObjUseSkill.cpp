...
int CObjUseSkill::GetAddUseMana(LPOBJ lpObj, CMagicInf * lpMagic)
{
	...
	if (lpObj->Class == ELF && lpObj->Type == OBJ_USER && lpObj->ChangeUP == 1 && lpObj->m_iMuseElfInfinityArrowSkillTime > 0)
	{
...
void CObjUseSkill::UseSkill(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex)
{
	...
	if (lpObj->pInventory[0].IsItem() == FALSE && lpMagic != 0)
	{
		if (lpMagic->m_Skill == AT_SKILL_WHEEL)
		{
			if (lpObj->Class == KNIGHT || lpObj->Class == GLADIATOR)
			{
				Function.MsgOutput(lpObj->m_Index, 1, "Utilize uma arma válida.");
				return;
			}
		}
	...
	if (usemana >= 0)
	{
		...
		{
			LogAdd("[%] ½ºÅ³ µô·¹ÀÌ½Ã°£ ¿À¹ö", gObj[aIndex].AccountID); //HermeX Fix
			return;
		}
...
void CObjUseSkill::UseSkill(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	if (lpObj->pInventory[0].IsItem() == FALSE && lpMagic != 0)
	{
		if (lpMagic->m_Skill == AT_SKILL_STRIKE)
		{
			Function.MsgOutput(lpObj->m_Index, 1, "Utilize uma arma válida.");
			return;
		}
	...
	if (usemana >= 0)
	{
		if (!gObj[aIndex].SkillDelay.Check(lpMagic->m_Skill))
		{
			LogAdd("[%] ½ºÅ³ µô·¹ÀÌ½Ã°£ ¿À¹ö", gObj[aIndex].AccountID);
			return;
		}
...
void CObjUseSkill::MaGumSkillDefenseDown(int aIndex, int aTargetIndex, int skill_level)
{
	...
	if (lpObj->Type == OBJ_USER && lpObj->Class != GLADIATOR)
	{
		return;
	}

	LogAdd("¸¶°Ë»ç ¸ó½ºÅÍ¹æ¾î·Â°¨¼Ò : %d", 10); // HermeX Fix
...
void CObjUseSkill::KnightSkillAddLife(int aIndex, int skill_level)
{
	...
	int iaddLifepower = (lpObj->Vitality + lpObj->AddVitality) / Buff._GreaterFortitude[0] + (lpObj->Energy + lpObj->AddEnergy) / Buff._GreaterFortitude[1] + viewpercent;
	int iLifeTime = Buff._GreaterFortitude[2] * 60;
	int iaddlife;
...
void CObjUseSkill::WizardMagicDefense(int aIndex, int aTargetIndex, int skill_level)
{
	...
	if (lpObj->Class != WIZARD && lpObj->Class != GLADIATOR)
	{
		return;
	}
	...
	lpTargetObj->m_WizardSkillDefense = (lpObj->Dexterity + lpObj->AddDexterity) / Buff._ManaShield[0] + (lpObj->Energy + lpObj->AddEnergy) / Buff._ManaShield[1];
	lpTargetObj->m_WizardSkillDefenseTime = Buff._ManaShield[2] * 60;
...
void CObjUseSkill::SkillDefense(int aIndex, int aTargetIndex, int skill_level)
{
	...
	lpTargetObj->m_SkillDefense = (lpObj->Energy + lpObj->AddEnergy) / Buff._GreaterDefense[0];
	lpTargetObj->m_SkillDefenseTime = Buff._GreaterDefense[1] * 60;
...
void CObjUseSkill::SkillAttack(int aIndex, int aTargetIndex, int skill_level)
{
	...
	lpTargetObj->m_SkillAttack = (lpObj->Energy + lpObj->AddEnergy) / Buff._GreaterDamage[0];
	lpTargetObj->m_SkillAttackTime = Buff._GreaterDamage[1] * 60;
...
void CObjUseSkill::SkillHealing(int aIndex, int aTargetIndex, int skill_level)
{
	...
	lpTargetObj->Life += (lpObj->Energy + lpObj->AddEnergy) / 5;
...
void CObjUseSkill::SkillBlowOfFury(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex, BOOL isCombo) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int tObjNum;
	int count = 0;
...
void CObjUseSkill::SkillWheel(int aIndex, CMagicInf* lpMagic, int aTargetIndex, BOOL isCombo) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int tObjNum;
	int count = 0;
...
void CObjUseSkill::SkillPowerSlash(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE targetangle, int aTargetIndex) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int StartDis = 1;
	int tObjNum;
...
int CObjUseSkill::SkillDeathPoison(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int tObjNum;
	int count = 0;
...
int CObjUseSkill::SkillSuddenIce(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int tObjNum;
	int count = 0;
...
BOOL CObjUseSkill::SkillHellFire2(int aIndex, int aTargetIndex, CMagicInf * lpMagic) //Identical
{
	if (aTargetIndex == 58)
	{
	...
	{
		LogAdd("[%s][%s] ÇïÆÄÀÌ¾î2 ¹öÆ° ´­·¯ÁöÁöµµ ¾Ê¾Ò´Âµ¥ ¿Ï·á°¡ µÇ¾ú´Ù", lpObj->AccountID, lpObj->Name); //HermeX Fix
		return false;
	}
...
BOOL CObjUseSkill::SkillHellFire2Start(int aIndex, CMagicInf * lpMagic)
{
	...
	if (lpObj->SkillHellFire2State != 0)
	{
		LogAdd("[%s][%s] ÇïÆÄÀÌ¾î2 ¹öÆ° ´­·¯Á³´Âµ¥ ¶Ç ´­·¯Á³´Ù´Â°Å ¿Ô´Ù", lpObj->AccountID, lpObj->Name); // HermeX Fix
		return false;
	}
...
BOOL CObjUseSkill::SkillDarkHorseAttack(int aIndex, int aTargetIndex, CMagicInf * lpMagic) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int tObjNum;
	int count = 0;
...
void CObjUseSkill::SkillAddCriticalDamage(int aIndex, int skill_level)
{
	...
	int addcriticaldamagevalue = (lpObj->Leadership + lpObj->AddLeadership) / Buff._GreaterCritical[0] + (lpObj->Energy + lpObj->AddEnergy) / Buff._GreaterCritical[1];
	int SkillTime = Buff._GreaterCritical[2] * 60;
	...
	if (partynum == -1)
	{
		...
		MsgOutput(aIndex, lMsg.Get(1240), (SkillTime / 60));
...
void CObjUseSkill::SkillElectricSpark(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
...
int CObjUseSkill::SkillInfinityArrow(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	if (lpObj->Class != ELF || lpObj->ChangeUP == 0)
	{
		return false;
	}
	...
	LogAdd("[%s][%s] Use Infinity Arrow Skill (Time:%d)(Character Level : %d)(ChangeUp: %d)", lpObj->AccountID, lpObj->Name, lpObj->m_iMuseElfInfinityArrowSkillTime, lpObj->Level, lpObj->ChangeUP);
	return true;
}
...
#if(GS_CASTLE==1)
				else if (gObj[iTarObjNum].Type != OBJ_NPC && gObj[iTarObjNum].MapNumber == MAP_INDEX_CASTLESIEGE && g_CastleSiege.GetCastleState() == 7)
				{
					bEnableAttack = 1;
				}
#endif
