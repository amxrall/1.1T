...
BOOL gObjMonsterRegen(LPOBJ lpObj) //Identical (HermeX Fix Applied)
{
	int map[3];
...
BOOL gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty) //Identical
{
...
int gObjMonsterSearchEnemy(LPOBJ lpObj, BYTE objtype)
{
	...
	for (n = 0; n < MAX_VIEWPORT_MONSTER; n++)
	{
		...
		if (tObjNum >= 0)
		{
			...
			{
				//DevGamesX 18/11/2018 - BotSystem
				if ((gObj[tObjNum].Class >= 100 && gObj[tObjNum].Class < 110) || (gObj[tObjNum].Type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon >= 0) || (gObj[tObjNum].IsBot > 1))
				{
	...
	case 1:
		...
		if (lpObj->m_RecallMon >= 0)
		{
			...
			if (lpObj->IsBot == 1)
			{
				...
				LogAdd("[BotPet] [%s][%s] BotPet has died [%s][%d][%d/%d]! ",
				...
				{
					GCServerMsgStringSend("El Invocador ha muerto, por favor crea uno nuevo.", lpObj->BotNumOwner, 1);
				...
				{
					char sbuf[512] = { 0 };
					wsprintf(sbuf, "A tu Invocador le quedan: %d vidas.", (lpObj->BotMaxLife - lpObj->BotLife));
					GCServerMsgStringSend(sbuf, lpObj->BotNumOwner, 1);
					//botPet.DieClose(lpObj->BotNumOwner, lpObj);
				}
			}
			else
#endif
...
void gObjMonsterProcess(LPOBJ lpObj) //Identical
{
	gObjMsgProc(lpObj);
	...
	//SirMaster Decompilation Here 100%
#if(GS_CASTLE==1)
	if (lpObj->Class == 283)
	{
		return;
...
void gObjMonsterMagicAttack(LPOBJ lpObj, int iMonsterClass)
{
	...
	pCount.h.c = 0xC1;
	pCount.h.headcode = PROTO_BEATTACK;
	pCount.h.size = 0;
				...
				else
				{
					...
					if (lpObj->m_RecallMon >= 0)
					{
						if (gObj[lpObj->m_RecallMon].Connected > PLAYER_LOGGED)
						{
							...
							//DevGamesX 18/11/2018 - BotSystem
							if (lpRecallObj->m_Rest == FALSE && ((lpObj->IsBot == 1 && lpObj->BotFollowMe == 1) || lpObj->IsBot == 0))
							{
					...
					if (lpObj->IsBot >= 1)
					{
						if (lpObj->IsBot == 1 && lpObj->BotNumOwner)
						{
							...
							case 2:		//Bot Alchemist
							{
								if (!(rand() % 70))
								{
									ChatSend(lpObj, "Comercia conmigo y prueba mi magia, es tan desconocida como peligrosa.");
									GCActionSend(lpObj, 0x93, lpObj->m_Index, 0);
								}
							}
							break;
							case 3:		//Bot Buffer
							{
								if (!(rand() % 70))
								{
									ChatSend(lpObj, "Comercia conmigo y te darÈ la fuerza que necesitas.");
									GCActionSend(lpObj, 0x93, lpObj->m_Index, 0);
								}
							}
							break;
							...
							case 5:		//Bot Trader
							{
								if (!(rand() % 70))
								{
									ChatSend(lpObj, "°Comercia conmigo y saldr·s ganando!");
									GCActionSend(lpObj, 0x93, lpObj->m_Index, 0);
								}
							}
							break;
							case 6:		//Bot Vip Shop
							{
								if (!(rand() % 70))
								{
									ChatSend(lpObj, "Solo los miembros V.I.P obtienen mi beneficio.");
									GCActionSend(lpObj, 0x93, lpObj->m_Index, 0);
								}
							}
							break;
...
void gObjRefillMonsterHP(LPOBJ lpMonsterObj, int iRefillHPSec)
{
	...
	if (lpMonsterObj->Class == 275)
	{
		...
		if (lpMonsterObj->iObjectSecTimer > giKundunRefillHPTime)
		{
			...
			LogAdd("[KUNDUN] HP Log -> [%d]»∏∫π  [%d]»∏∫πΩ√∞£  [%7.0f]«ˆ¿ÁHP",
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj) //Identical
{
	int store_count = 0;
	int ExtDropPer = 0;
	int DropItemNum = 0;
	...
	Dropping.gObjDrop(lpObj, lpTargetObj);

#if(GS_CASTLE == 1)

	if (lpObj->m_btCsNpcType) //SirMaster Fixed
	{
		return;
	}
	...
	if (lpObj->Class == 295)
	{
		...
		for (int i = 0; i < iMaxNumOfRewardItem; i++)
		{
			...
			LogAdd("[Castle HuntZone] Boss Monster ItemDrop MaxHitUser [%d][%s][%s]", i, lpTargetObj->AccountID, lpTargetObj->Name);
...
BOOL gEventMonsterItemDrop(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ((rand() % 10000) < g_iKundunMarkDropRate)
	{
		...
		if (lpObj->Level < 25)
			...
			level = 6;
		else if (lpObj->Level > 115 && lpObj->Level < 400 + 1)
			level = 7;
		...
		else
		{
			if (lpObj->Level >= gSetItemInCastleHuntZoneDropLevel) //Good
			{
				...
				if ((rand() % 10000) < gSetItemInCastleHuntZoneDropRate)
				{
					if (OBJMAX_RANGE(MaxHitUser))
					{
						LogAdd("[Castle HuntZone] Drop SetItem [%s][%s][%d]", gObj[MaxHitUser].AccountID, gObj[MaxHitUser].Name, gSetItemInCastleHuntZoneDropRate);
...
void CQeustNpcTeleport::Run(int aIndex)
{
	...
	if (this->TimeCount > gQuestNPCTeleportTime)
	{
		...
		LogAdd("NPC %d∑Œ ¿Ãµø", tableindex);	// #warning Translation Deathway
	}
}