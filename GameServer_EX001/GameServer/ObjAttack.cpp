...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic, int magicsend, unsigned char MSBFlag, int AttackDamage, BOOL bCombo) //Identical
{
	int skillSuccess = 0;
	LPOBJ lpCallObj;
	...
	if (AttackDamage == 0)
	{
			...
			|| skill == 74)
		{
			...
			if (Balancer._Active == true)
			{
				Balancer.Attack(lpObj, lpTargetObj, lpMagic, AttackDamage);
			}
		...
		{
			if ((lpObj->Class == WIZARD || lpObj->Class == GLADIATOR) && skill)
			{
		...
		if (gObjWingSprite(lpObj) == TRUE)
		{
			...
			if (lpObj->Class == WIZARD || lpObj->Class == ELF)
			{
				lpObj->Life -= 1.0f;
			}
			else
			...
			if (skill == 76)
			{
				if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER)
				{
					if (AttackDamage > 0)
					{
						...
						if (lpObj->m_btCsJoinSide == FALSE || (lpObj->m_btCsJoinSide != lpTargetObj->m_btCsJoinSide)) //possible bad decompilation fix??
						{
	...
	if (AttackDamage >= 1)
	{
		...
		if (lpTargetObj->m_SkillHarden)
		{
			...
			if (lpTargetObj->m_SkillHarden <= 0)
			{
				...
				lpTargetObj->m_ViewSkillState &= 0xFFFFFFDF; //HermeX Fix ??
	...
	if (lpObj->Class == ELF && lpObj->Level == 1 && AttackDamage > 10)
	{
	...
	if (AttackDamage > 0)
	{
		...
		if (bCombo)
		{
			...
			if (lpObj->Class == KNIGHT)
			{
				...
				{
				case KNIGHT:
					iComboDamage = (Strength + Dexterity + Energy) / Balancer.Combo[0]; break;
				case WIZARD:
					iComboDamage = (Strength + Dexterity + Energy) / Balancer.Combo[1]; break;
				case ELF:
					iComboDamage = (Strength + Dexterity + Energy) / Balancer.Combo[2]; break;
				case GLADIATOR:
					iComboDamage = (Strength + Dexterity + Energy) / Balancer.Combo[3]; break;
				case LORD:
					iComboDamage = (Strength + Dexterity + Energy) / Balancer.Combo[4]; break;
				default:
					iComboDamage = (Strength + Dexterity + Energy) / 2; break;
				}
			}
		...
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER)
		{
			LogAdd("[PvP System] Victim:[%s][%s], Attacker:[%s][%s] - HP[%f] -> HP[%f]",
				lpTargetObj->AccountID, lpTargetObj->Name, lpObj->AccountID, lpObj->Name,
				lpTargetObj->Life + AttackDamage, lpTargetObj->Life);
		}
	}
	else
	{
...
int CObjAttack::GetAttackDamage(LPOBJ lpObj, int targetDefense, BYTE& effect, BOOL bIsOnDuel, CMagicInf* lpMagic)
{
	...
	if (bTwoHandWeapon)
	{
		if (bIsOnDuel)
		{
			ad = (lpObj->m_AttackDamageRight + lpObj->m_AttackDamageLeft) * 100 / 100 - targetDefense;
		}
		else
		{
		...
		Left->m_Type == ITEMGET(4, 20) || Left->m_Type == ITEMGET(4, 21))
	{
		if (bIsOnDuel)
		{
			ad = (lpObj->m_AttackDamageLeft) * 100 / 100 - targetDefense;
		}
		else
		{
		...
		(Right->m_Type >= ITEMGET(4, 16) && Right->m_Type < ITEMGET(5, 0)))
	{
		if (bIsOnDuel)
		{
			ad = (lpObj->m_AttackDamageRight) * 100 / 100 - targetDefense;
		}
		else
		{
...
int CObjAttack::GetAttackDamageWizard(LPOBJ lpObj, int targetDefense, CMagicInf* lpMagic, BYTE& effect, BOOL bIsOnDuel)
{
	...
	__try
	{
		if (bIsOnDuel)
		{
			ad = (damagemin + (rand() % (subd + 1))) * 100 / 100 - targetDefense;	// #formula
		}
		else
		{
		...
		if (lpObj->m_CriticalDamage > 0)
		{
			if ((rand() % 100) < lpObj->m_CriticalDamage)
			{
				if (bIsOnDuel)
				{
					ad = damagemax * 100 / 100 - targetDefense;
				}
				else
				{
