	...
	if (lpObj->MapNumber == gObj[this->m_iTargetIndex].MapNumber)
	{
		if (gObj[this->m_iTargetIndex].Life > 0.0f)
		{
			...
			if (dis < RAVEN_ATTACK_DISTANCE)
			{
				...
				{
					this->SendAttackMsg(lpObj->m_Index, this->m_iTargetIndex, CriticalDamage, 1);
				}
				else
				{
...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int criticaldamage, int iActionType) //Identical
{
	int skillSuccess = 0;
	LPOBJ lpCallObj;
	...
	if (g_CastleSiege.GetCastleState() == 7)
	{
		if (lpObj->m_btCsJoinSide > 0)
		{
			...
			{
				return FALSE; //Impedir Ataque do Dark Spirit no mesmo time
			}
		}
	}
#endif
	...
	if (lpObj->pInventory[1].m_DurabilitySmall >= 800)
	{
		...
		if (lpObj->pInventory[1].m_Durability < 1.0f)
		{
			...
			LogAdd("[%s][%s] 다크스피릿의발톱 Item is Broken because durability is exhausted [%d]",
	...
	if (AttackDamage >= 1)
	{
		...
		if (lpTargetObj->m_SkillHarden)
		{
			...
			if (lpTargetObj->m_SkillHarden <= 0)
			{
				...
				lpTargetObj->m_ViewSkillState &= 0xFFFFFFDF;
...
int  CDarkSpirit::GetAttackDamage(LPOBJ lpObj, int targetDefense, BOOL bIsOnDuel, int crititcaldamage)
{
	...
	if (bIsOnDuel)
	{
		ad = AttackDamage * 100 / 100 - targetDefense;
	}
	else
	{
