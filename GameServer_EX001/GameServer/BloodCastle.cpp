...
{
	g_bBloodCastle = GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "BloodCastleEvent", 0, COMMONSERVER_FILE);
	g_iAngelKingsPaperDropRate = GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "AngelKingsPaperDropRate", 0, COMMONSERVER_FILE);
	g_iBloodBoneDropRate = GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "BloodBoneDropRate", 0, COMMONSERVER_FILE);
	g_iStoneDropRate = GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "StoneDropRate", 0, COMMONSERVER_FILE);
	g_bStoneItemDrop = GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "StoneItemDrop", 0, COMMONSERVER_FILE);
}
...
int  CBloodCastle::CheckEnterLevel(int iIndex, int iLevel)	// RET : [2:Error][1:OverLevel][0:InLevel - Success][-1:UnderLevel]
{
	...
	if (gObj[iIndex].Class == LORD || gObj[iIndex].Class == GLADIATOR)
	{
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	LogAdd("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Start (Account:%s, Name:%s, Level:%d)",
	...
	if (iMIX_SUCCESS_RATE < 0 || iMIX_SUCCESS_RATE > 100)
	{
		...
		LogAdd("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - MixRate Out of Bound (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	{
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - MixMoney < 0 (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ((gObj[iIndex].Money - iMIX_NEED_MONEY) < 0)	// Not enoght zen
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd("[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - Not Enough Money (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ((rand() % 100) < iMIX_SUCCESS_RATE)	// Siccess
	{
		...
		LogAdd("[Åõ¸í¸ÁÅä Mix] [%s][%s] CBMix Success %d Money : %d-%d",
	...
	else	// Failure
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd("[Åõ¸í¸ÁÅä Mix] [%s][%s] CBMix Fail %d Money : %d-%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
...
void CBloodCastle::DropChaosGem(int iIndex)
{
	...
	{
	case 11:
		if (BloodEvent._Resets[0] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[0], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 0);
		break;
	case 12:
		if (BloodEvent._Resets[1] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[1], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 1);
		break;
	case 13:
		if (BloodEvent._Resets[2] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[2], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 2);
		break;
	case 14:
		if (BloodEvent._Resets[3] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[3], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 3);
		break;
	case 15:
		if (BloodEvent._Resets[4] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[4], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 4);
		break;
	case 16:
		if (BloodEvent._Resets[5] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[5], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 5);
		break;
	case 17:
		if (BloodEvent._Resets[6] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", BloodEvent._Resets[6], gObj[iIndex].Name);
		}

		BloodEvent.Item(iIndex, 6);
		break;
	default:
		break;
	}
