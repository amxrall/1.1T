...
int CDevilSquare::gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg)
{
	...
	exp += Setting._Experience[lpObj->Custom->VipCount];
		...
		if (number >= 0)
		{
			...
			if (lpTargetObj->MapNumber == lpPartyObj->MapNumber)
			{
				...
				if (dis[n] < 10)
				{
					...
					if (toplevel >= (lpPartyObj->Level + 400)) // #formula
					{
						totallevel = totallevel + lpPartyObj->Level + 400;
					}
					else
					{
				...
				if (dis[n] < 10)
				{
					...
					if (lpPartyObj->Type == OBJ_USER)
					{
						if (exp > 0)
						{
							...
							if (gObjLevelUp(lpPartyObj, exp, lpTargetObj->Class, EVENT_TYPE_PARTY) == 0)
							{
								continue;
							}
						}
