...
void DGGuildMemberInfo(SDHP_GUILDMEMBER_INFO * lpMsg) //Identical
{
	...
	for (int n = OBJ_STARTUSERINDEX; n < OBJMAX; n++)
	{
		if (gObj[n].Connected > PLAYER_LOGGED)
		{
			if (gObj[n].Name[0] == lpMsg->MemberID[0])
			{
				...
				{
#if(GS_CASTLE==1) //HermeX Fix
					gObj[n].m_ViewSkillState &= 0xFFF3FFFF; //HermeX Fix
					gObj[n].m_ViewSkillState &= 0xFFBFFFFF; //HermeX Fix
					gObj[n].m_ViewSkillState &= 0xFF7FFFFF; //HermeX Fix
