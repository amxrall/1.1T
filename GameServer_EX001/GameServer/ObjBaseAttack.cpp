...
{
	if (lpObj->Class == ELF && lpObj->Type == OBJ_USER)
	{
			...
			Right->m_Type == ITEMGET(4, 19))
		{
			...
			Left->m_Type == ITEMGET(4, 17) || Left->m_Type == ITEMGET(4, 20) ||
...
BOOL CObjBaseAttack::CheckAttackArea(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if (lpObj->Type == OBJ_USER)
	{
		...
		if (iRet != 0)
		{
			LogAdd("[%s][%s] Try Attack In Not Attack Area (%s,%d,%d) errortype = %d", lpObj->AccountID, lpObj->Name, lMsg.Get(MSGGET(7, 208) + lpObj->MapNumber), lpObj->X, lpObj->Y, iRet);
		}
	}
...
BOOL CObjBaseAttack::PkCheck(LPOBJ lpObj, LPOBJ lpTargetObj) //Identical
{
	if (lpObj->Type == OBJ_USER)
	{
...
BOOL CObjBaseAttack::ResistanceCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int skill) //Identical
{
...
BOOL CObjBaseAttack::MissCheckPvP(LPOBJ lpObj, LPOBJ lpTargetObj, int skill, int skillSuccess, int magicsend, BOOL & bAllMiss)
{
	...
	if (lpTargetObj->Class == KNIGHT)
	{
		iDefenseRate = lpTargetObj->Level * 2 + lpTargetObj->Dexterity / 2;	// #formula
	}
	else if (lpTargetObj->Class == LORD)
	{
		iDefenseRate = lpTargetObj->Level * 2 + lpTargetObj->Dexterity / 2;	// #formula
	}
	else if (lpTargetObj->Class == ELF)
	{
		iDefenseRate = lpTargetObj->Level * 2 + lpTargetObj->Dexterity / 10;	// #formula
	}
	else if (lpTargetObj->Class == GLADIATOR)
	{
		iDefenseRate = lpTargetObj->Level * 2 + lpTargetObj->Dexterity / 4;	// #formula
	}
	else if (lpTargetObj->Class == WIZARD)
	{
		iDefenseRate = lpTargetObj->Level * 2 + lpTargetObj->Dexterity / 4;	// #formula
	}

	if (iAttackRate <= 0.0f || iDefenseRate <= 0.0f || lpObj->Level <= 0 || !lpTargetObj)
		return FALSE;
	...
	if (dwRate > iAttackSuccessRate)
	{
		...
		/*if ( g_bShieldComboMissOptionOn == TRUE )
		{
			...
			{
				LogAdd("[Shield] ComboSkill Cancel! [%s][%s]", lpObj->AccountID, lpObj->Name);
