...
void ProtocolCore(BYTE protoNum, unsigned char * aRecv, int aLen, int aIndex, BOOL Encrypt, int serial)
{
	...
	else
	{
		...
		case 0x40:
		     {
				if (gObj[aIndex].Custom->InEvent == true || DeathEvent.PlayerStruct[aIndex]._InEvent == true 
					|| TheftEvent.PlayerStruct[aIndex]._InEvent == true)
				{
					Function.MsgOutput(aIndex, 1, "Proibido formar party em evento.");
					return;
				}
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex)
{
	if (Setting._Checksum)
	{
		...
		{
			LogAdd("error-L1 : CheckSum-Exe error %d %d %d [%s]", dwgCheckSum[gObj[aIndex].CheckSumTableNum], lpMsg->m_dwKey, gObj[aIndex].CheckSumTableNum, gObj[aIndex].AccountID);
			CloseClient(aIndex);
			return;
		}
	}
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex)
{
	...
	{
		LogAdd("[Anti-HACK][PChatProc][%s][%s] Chat Message Len : %d", lpObj->AccountID, lpObj->Name, slen);
		return;
	}
	...
	case '!':	// Global Announcement
		if (slen > 2)
		{
			if (gObj[aIndex].Authority > 1)
			{
				if (lpChat->chatmsg[1] == '!')
				{
					sprintf_s(Buffer, "[%s] %s:%s", szServerName, gObj[aIndex].Name, &lpChat->chatmsg[2]);

					Function.SendMapServerGroupMsg(Buffer);
				}
				else
				{
					Function.AllServerAnnounce("%s:%s", gObj[aIndex].Name, &lpChat->chatmsg[1]);
				}

				LogAdd(lMsg.Get(MSGGET(1, 215)), gObj[aIndex].AccountID, gObj[aIndex].Name, &lpChat->chatmsg[1]);
				return;
			}
		}
		break;
	case '/':	// Command
		if (slen > 2)
		{
			Command.Management(lpChat, aIndex);
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex)
{
	...
	char Account[MAX_ACCOUNT_LEN + 1]; char Serial[17];
	Account[MAX_ACCOUNT_LEN] = 0; Serial[16] = 0;
	memcpy(Serial, lpMsg->ClientSerial, sizeof(lpMsg->ClientSerial));
	memcpy(Account, lpMsg->Account, MAX_ACCOUNT_LEN);
	BuxConvert(Account, MAX_ACCOUNT_LEN);

	if (lpMsg->ClientVersion[0] != szClientVersion[0] ||
		lpMsg->ClientVersion[1] != szClientVersion[1] ||
		lpMsg->ClientVersion[2] != szClientVersion[2] ||
		lpMsg->ClientVersion[3] != szClientVersion[3] ||
		lpMsg->ClientVersion[4] != szClientVersion[4])
	{
	...
	if (strcmp(Serial, Setting._Serial) != 0)
	{
		LogAddC(2, "error-L1: Serial error [%s] [%s]", Account, Serial);
	...
	if (PacketCheckTime(lpObj) == FALSE)
	{
		LogAdd(lMsg.Get(MSGGET(1, 217)), aIndex, Account);
		CloseClient(aIndex);
		return;
	}
	...
	{
		if (gObj[i].Connected == PLAYER_PLAYING && Attack.PlayerStruct[i]._OffAttack == true)
		{
	...
	if (gObj[aIndex].Connected != PLAYER_CONNECTED)
	{
		LogAdd(lMsg.Get(MSGGET(1, 218)), aIndex, Account);
		CloseClient(aIndex);
		return;
	}
	...
	if (gObj[aIndex].LoginMsgSnd != FALSE)
	{
		...
		{
			LogAdd(lMsg.Get(MSGGET(1, 219)), aIndex, Account, gObj[aIndex].Ip_addr);
		}
		else
		{
			LogAddC(2, "error : %d %s %d", gObj[aIndex].LoginMsgSnd, __FILE__, __LINE__);
		}

		return;
	}

	lpObj->Custom->VipCount = Manager.VipCount(Account);
		
	if (lpObj->Custom->VipCount < Setting._Type)
	{
	...
	PHeadSetB((LPBYTE)&Result, 0x01, sizeof(Result));
	Result.Number = aIndex;
	memcpy(Result.Account, lpMsg->Account, sizeof(Result.Account));
	memcpy(Result.Password, lpMsg->Password, sizeof(Result.Password));
	strcpy(Result.IpAddress, gObj[aIndex].Ip_addr);
	...
	lpObj->LoginMsgSnd = 1;
	lpObj->LoginMsgCount++;
	memcpy(lpObj->AccountID, Account, MAX_ACCOUNT_LEN);
	lpObj->AccountID[MAX_ACCOUNT_LEN] = 0;
	lpObj->m_cAccountItemBlock = 0;

	wsJServerCli.DataSend((char*)&Result, Result.h.size);
...
void CGClientMsg(PMSG_CLIENTMSG* lpMsg, int aIndex)
{
	if (lpMsg->Flag)
	{
		LogAddC(2, "error-L1 : HackCheck [%s][%s] 0x%x 0x%x", gObj[aIndex].AccountID, gObj[aIndex].Name, lpMsg->Flag, lpMsg->subFlag);
	}
	else
	{
...
void CGPCharacterCreate(PMSG_CHARCREATE * lpMsg, int aIndex)
{
	...
	{
		LogAdd("[Anti-HACK][CGPCharacterCreate] Attempted Character Create during GamePlay. [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
...
void CGPCharDel(PMSG_CHARDELETE * lpMsg, int aIndex)
{
	...
	if (gObj[aIndex].Connected == PLAYER_PLAYING)
	{
		LogAdd("[Anti-HACK][CGPCharDel] Attempted Character Delete during GamePlay. [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
	...
	{
		LogAdd("[Anti-HACK][CGPCharDel] Attempted Character Delete during Prevented Time. [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
	...
	if (Setting._Personal == 0 && gObjJoominCheck(aIndex, szJoomin) == FALSE)
	{
		pResult.result = 2;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		return;
...
void GCLevelUpMsgSend(int aIndex, int iSendEffect)
{
	...
	pMsg.Strength = gObj[aIndex].Strength;
	pMsg.Dexterity = gObj[aIndex].Dexterity;
	pMsg.Vitality = gObj[aIndex].Vitality;
	pMsg.Energy = gObj[aIndex].Energy;
	pMsg.Leadership = gObj[aIndex].Leadership;
...
void CGItemGetRequest(PMSG_ITEMGETREQUEST * lpMsg, int aIndex)
{
	...
	{
		LogAdd("[%s][%s] CGItemGetRequest() Failed : Transaction == 1, IF_TYPE : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
		return;
	}
					...
					if (MAIN_INVENTORY_RANGE(pos))
					{
						if (MapC[map_num].ItemGive(aIndex, item_num, true) == TRUE)
						{
							...
							LogAdd(lMsg.Get(MSGGET(1, 221)), gObj[aIndex].AccountID, gObj[aIndex].Name, map_num,
								...
								NewOption[6], lpItem->m_SetOption, 0,
								0, 0);
	...
	if (pResult.Result == 1)
	{
		...
		if (bIsItem == TRUE)
		{
			...
			if (type == ITEMGET(14, 11)) // Box of Luck until Kunduns
			{
				...
				LogAdd("[%s][%s] Used box of luck (level:%d)", lpObj->AccountID, lpObj->Name, level);
			}
			else if (type == ITEMGET(14, 42))
			{
			    Golden.Item(aIndex);
...
void CGTalkRequestRecv(PMSG_TALKREQUEST * lpMsg, int aIndex) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int DealerNumber;
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos)
{
	...
	//if (itemtype >= ITEMGET(14, 0) || (itemtype >= ITEMGET(13, 0) && itemtype < ITEMGET(13, 4)) || itemtype == ITEMGET(13, 10) || (itemtype >= ITEMGET(12, 7) && itemtype < ITEMGET(12, 100)) || itemtype == ITEMGET(4, 7) || itemtype == ITEMGET(4, 15))
	...
	else
	{
		...
		DurItem->Convert(DurItem->m_Type, DurItem->m_Option1, DurItem->m_Option2, DurItem->m_Option3,
			DurItem->m_NewOption, DurItem->m_SetOption, 0, CURRENT_DB_VERSION);
...
void CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex)
{
	...
	{
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, TRUE);
...
void CGPShopReqBuyList(PMSG_REQ_BUYLIST_FROM_PSHOP * lpMsg, int aSourceIndex)
{
	...
	if (lpObj->Type != OBJ_USER && lpObj->IsBot != 4)
	{
	...
	{
		LogAdd("[Fix Inv.Ptr] False Location - CGPShopReqBuyItem%s, %d", __FILE__, __LINE__);
	}
	...
	if (lpObj->IsBot == 4)
	{
		...
		/*	int number = BotStore.bot[number].index;
		if (BotStore.bot[number].OnlyVip == 1 && gObj[aSourceIndex].m_Vip == 0)
		{
			ChatTargetSend(&gObj[BotStore.bot[number].index], "Tienes que ser vip para poder comprarme!", aSourceIndex);
			LogAdd( "[BotTrader][Bot:%d](%s) Account is not VIP", number, gObj[aSourceIndex].AccountID);
			return;
		}*/

		//int index = lpObj->m_Index;
		char sbuf[100] = { 0 };
		/*if (lpObj->BotDefense == 1 && gObj[aSourceIndex].m_Vip == 0)
		{
			LogAdd( "[PShop] [%s][%s] PShop Item Buy Request Failed : Seller is not VIP (%d)"
				,gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL) );

			GCServerMsgStringSend("Esta tienda personal es únicamente para miembros V.I.P",aSourceIndex, 1);
			return;
		}*/
		wsprintf(sbuf, "Bem vindo a minha loja, eu sou o comerciante %s", lpObj->Name);
		GCServerMsgStringSend(sbuf, aSourceIndex, 0);
		switch (lpObj->BotPower)
		{
		case 0:
		{
			GCServerMsgStringSend("A loja pessoal usa zen.", aSourceIndex, 0);
		}break;
		case 1:
		{
			GCServerMsgStringSend("A loja pessoal usa Gold.", aSourceIndex, 0);
			//GCMoneySend(aSourceIndex, gObj[aSourceIndex].m_Gold);

			wsprintf(sbuf, "Moeda Gold = %d", gObj[aSourceIndex].m_Gold);
			GCServerMsgStringSend(sbuf, aSourceIndex, 1);
		}break;
		case 2:
		{
			GCServerMsgStringSend("A loja pessoal usa Cash.", aSourceIndex, 0);
			//GCMoneySend(aSourceIndex, gObj[aSourceIndex].Cash);

			wsprintf(sbuf, "Moeda Cash = %d", gObj[aSourceIndex].m_Cash);
			GCServerMsgStringSend(sbuf, aSourceIndex, 1);
		}break;
		}
	...
	else
	{
		...
		__try
		{
			...
			if (lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE)
			{
				...
				if (gObj[aSourceIndex].Money < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue) //TIENDA PERSONAL
				{
				...
				if (lpObj->IsBot == 0)
				{
					...
					case 1:
					{
						if (gObj[aSourceIndex].m_Gold < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
						{
							LogAdd( "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of PCPoints",
								gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
								lpObj->AccountID, lpObj->Name);
							wsprintf(sbuf, "Você não tem Golds suficiente");
							GCServerMsgStringSend(sbuf, aSourceIndex, 1);
							//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
					...
					case 2:
					{
						if (gObj[aSourceIndex].m_Cash < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
						{
							LogAdd( "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Vip Money",
								gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
								lpObj->AccountID, lpObj->Name);
							wsprintf(sbuf, "Você não tem Cash suficiente");
							GCServerMsgStringSend(sbuf, aSourceIndex, 1);
							//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
				...
				else
				{
					...
					case 1:
					{
						gObj[aSourceIndex].m_Gold -= dwCost;
						Manager.ExecFormat("UPDATE MEMB_INFO SET Gold = Gold - %d WHERE memb___id =  '%s'", dwCost, gObj[aSourceIndex].AccountID);
						::GCMoneySend(aSourceIndex, gObj[aSourceIndex].m_Gold);
						//gObj[aSourceIndex].Money -= dwCost;
						//lpObj->Money += dwCost;
						//::GCMoneySend(aSourceIndex, gObj[aSourceIndex].Money);
					...
					case 2:
					{
						gObj[aSourceIndex].m_Cash -= dwCost;
						Manager.ExecFormat("UPDATE MEMB_INFO SET Cash = Cash - %d WHERE memb___id =  '%s'", dwCost, gObj[aSourceIndex].AccountID);
						::GCMoneySend(aSourceIndex, gObj[aSourceIndex].m_Cash);
						//gObj[aSourceIndex].Money -= dwCost;
						//lpObj->Money += dwCost;
						//::GCMoneySend(aSourceIndex, gObj[aSourceIndex].Money);
						//::GCMoneySend(lpObj->m_Index, lpObj->Money);
...
void CGGuildRequestRecv(PMSG_GUILDJOINQ * lpMsg, int aIndex) //Identical
{
	...
	LogAdd("[%s][%s] Guild Request - Join : %s", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[number].lpGuild->Name);
}

void CGGuildRequestResultRecv(PMSG_GUILDQRESULT * lpMsg, int aIndex) //Didn't change on GS_CS 18
{
	int number;
	BYTE result = 0;
	int pnumber = -1;
...
void CGGuildDelUser(PMSG_GUILDDELUSER * lpMsg, int aIndex)
{
	...
	if (!strcmp(gObj[aIndex].lpGuild->Names[0], gObj[aIndex].Name))
	{
		...
		if (Setting._Personal == 1 || gObjJoominCheck(aIndex, joomin) == TRUE)
		{
			...
			LogAdd("[%s][%s] Guild Request - Dismiss All : %s", gObj[aIndex].AccountID, gObj[aIndex].Name, guildname);
		}
	}
	else
	{
		...
		LogAdd("[%s][%s] Guild Request - Leave : %s", gObj[aIndex].AccountID, gObj[aIndex].Name, guildname);
	}
}
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type) //Identical
{
	PMSG_GUILDWARREQUEST_RESULT pMsg;//-4
	_GUILD_INFO_STRUCT * lpMyGuild;//-8
	char _guildname[MAX_GUILD_LEN + 1];//-C
	_GUILD_INFO_STRUCT * lpNode;//-18
	int n;//-1C
	int warmaster;//-20
	...
	lpNode = Guild.SearchGuild(_guildname);
	...
	if (lpNode)
	{
		...
		n = 0;	// #warning Change this 0 to OBJ_STARTUSERINDEX
		warmaster = -1;
...
void GCGuildWarRequestSend(LPSTR GuildName, int aIndex, int type) //Identical
{
...
#if(GS_CASTLE==0)
void GCGuildWarRequestSendRecv(PMSG_GUILDWARSEND_RESULT * lpMsg, int aIndex) //Identical
{
...
#else
void GCGuildWarRequestSendRecv(PMSG_GUILDWARSEND_RESULT* lpMsg, int aIndex) //Need Check (Identical, Wrong Size & Locals)
{
	PMSG_GUILDWAR_DECLARE pMsg;
	int count;
	int g_call;
	_GUILD_INFO_STRUCT* lpMyNode;
...
void GCGuildWarDeclare(int aIndex, LPSTR _guildname) //Identical
{
	PMSG_GUILDWAR_DECLARE pMsg;
#if(GS_CASTLE==0)
	PHeadSetB((LPBYTE)&pMsg, 0x62, sizeof(pMsg));
...
void GCGuildWarEnd(int aIndex, BYTE result, char* _guildname) //Identical
{
...
void GCGuildWarScore(int aIndex) //Identical
{
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex) //Identical
{
	if (!gObjIsConnectedGP(aIndex))
	{
		LogAddC(2, "error-L3 [%s][%d]", __FILE__, __LINE__);
	...
	case CHAOS_TYPE_HT_BOX:
		...
		break;
	default:
		LogAdd("[%s][%s] Undefine chaosmix type detect %d", lpObj->AccountID, lpObj->Name, iMixType);
		break;
	}
}
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex) //Need Check (Identical but Weird Compilation as Deathway says)
{
	if (!OBJMAX_RANGE(aIndex))
	{
	...
	if (lpMove->X != gObj[aIndex].X || lpMove->Y != gObj[aIndex].Y)
	{
		if (Speed.SpeedStruct[aIndex]._Moviment > 0 && Speed._Display[2] == 1)
		{
			Function.MsgOutput(aIndex, 0, "Moviment: %d", Speed.SpeedStruct[aIndex]._Moviment);
		}
	...
	lpObj->TX = (BYTE)ax; //Fixed
	lpObj->TY = (BYTE)ay; //Fixed
	...
	if (lpObj->Type == 1)
	{
		...
		for (int i = 0; i < 256; i++)
		{
			...
			//Something Wrong Here (Missing MOV ECX, EDX & MOVZX AL, AL)
			attr = (WORD)MapC[lpObj->MapNumber].GetAttr(nextX, nextY);

			if (((WORD)attr & (WORD)16) == (WORD)16)
			{
				iResult = 1;
				break;
			}
	...
	if (BC_MAP_RANGE(lpObj->MapNumber) && lpObj->Type == OBJ_USER)
	{
		...
		case 0x02:
			...
			if (lpObj->TY > 76 && g_BloodCastle.m_BridgeData[(lpObj->MapNumber - 11)].m_bCASTLE_DOOR_LIVE)
			{
				lpObj->m_Rest = 1;
				lpObj->PathCur = 0;
				lpObj->PathCount = 0;
	...
	PHeadSetB((LPBYTE)&pMove, PROTO_WALK, sizeof(pMove));
...
void RecvPositionSetProc(PMSG_POSISTION_SET * lpMove, int aIndex)
{
	...
	PHeadSetB((LPBYTE)&pMove, PROTO_SETPOS, sizeof(pMove));
	pMove.NumberH = SET_NUMBERH(aIndex);
	pMove.NumberL = SET_NUMBERL(aIndex);
	pMove.X = lpMove->X;
	pMove.Y = lpMove->Y;
...
void GCDamageSend(int aIndex, int TargetIndex, int AttackDamage, int MSBFlag, int MSBDamage)
{
	...
	PHeadSetB((LPBYTE)&Result, PROTO_ATTACK, sizeof(Result));

	Result.NumberH = SET_NUMBERH(TargetIndex);
	Result.NumberL = SET_NUMBERL(TargetIndex);
	Result.DamageH = SET_NUMBERH(AttackDamage);
	Result.DamageL = SET_NUMBERL(AttackDamage);
	...
	{
		cManager.DataSend((LPBYTE)&Result, Result.h.size);
	}
...
struct PMSG_DAMAGE
{
	...
	/*<thisrel this+0x4>*/ /*|0x1|*/ unsigned char DamageH;
	/*<thisrel this+0x5>*/ /*|0x1|*/ unsigned char DamageL;
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	if (!gCheckSkillDistance(aIndex, usernumber, lpMagic->m_Skill))
	{
		...
		if (lpObj->m_iSkillDistanceErrorCount > g_iSkillDistanceKickCount)
		{
			...
			{
				LogAdd("[SKILL DISTANCE CHECK] [%s][%s] Kick Invalid Skill Area User. count(%d)", lpObj->AccountID, lpObj->Name, lpObj->m_iSkillDistanceErrorCount);
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex) //Identical
{
	if (!OBJMAX_RANGE(aIndex))
		return;
...
void CGBeattackRecv(unsigned char* lpRecv, int aIndex, int magic_send)
{
	...
	// Check the Protocol
	if (lpCount->h.headcode != PROTO_BEATTACK)
	{
		LogAdd("error-L3 %s %d", __FILE__, __LINE__);
		return;
	}
	...
	if (gObj[aIndex].Type == OBJ_USER)
	{
			...
			lpMagic->m_Skill) < 1)
		{
			...
			LogAddC(2, lMsg.Get(MSGGET(1, 239)), gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex) //Identical
{
	int pos;
	CItem * citem;
	...
	Function.UseItem(lpMsg, aIndex);

	if (Potion._Active[0] == true)
	{
		if (citem->m_Type >= ITEMGET(14, 0) && citem->m_Type <= ITEMGET(14, 6))
		{
			if (gObj[aIndex].Custom->Time == 1)
			{
				GCInventoryItemDeleteSend(aIndex, 255, 1);
				return;
			}
		}

		gObj[aIndex].Custom->Time = 1;
	}
	...
	if (citem->IsItem())
	{
		...
		else if (citem->m_Type == ITEMGET(14, 10)) // Town Portal Scroll
		{
			...
			else if (gObj[aIndex].MapNumber == MAP_INDEX_CASTLESIEGE)
			{
				...
				{
					if (g_CastleSiege.CheckCastleOwnerMember(aIndex) == TRUE || g_CastleSiege.CheckCastleOwnerUnionMember(aIndex) == TRUE) //DiG Fix
					{
						gObjMoveGate(aIndex, 101);
					}
					else
...
void GCReFillSend(int aIndex, int Life, BYTE Ipos, unsigned char flag)
{
	...
	Result.IPos = Ipos;
	Result.LifeH = SET_NUMBERH(Life);
	Result.LifeL = SET_NUMBERL(Life);
	Result.Flag = 0;
	Result.Life = Life;
...
void GCManaSend(int aIndex, int Mana, BYTE Ipos, unsigned char flag, int BP)
{
	...
	Result.IPos = Ipos;
	Result.ManaH = SET_NUMBERH(GET_MAX_WORD_VALUE(Mana));
	Result.ManaL = SET_NUMBERL(GET_MAX_WORD_VALUE(Mana));
	Result.BPH = SET_NUMBERH(GET_MAX_WORD_VALUE(BP));
	Result.BPL = SET_NUMBERL(GET_MAX_WORD_VALUE(BP));
	Result.Mana = Mana;
	Result.BP = BP;
...
{
	if (gObj[aIndex].Custom->VipCount == 3) // Potion
	{
		for (int i = 12; i < 76; i++)
		{
...
void GCReqmoveDevilSquare(PMSG_REQ_MOVEDEVILSQUARE* lpMsg, int aIndex)
{
	...
	if (lpObj->Class == LORD || lpObj->Class == GLADIATOR)
	{
		level = (level + 1) / 2 * 3;
	}
...
void CGRequestEnterChaosCastle(PMSG_REQ_MOVECHAOSCASTLE* lpMsg, int iIndex)
{
	...
	if (gObj[iIndex].Class == ELF && gObj[iIndex].m_RecallMon >= 0)
	{
...
void CGReqMoveOtherServer(PMSG_REQ_MOVE_OTHERSERVER * lpMsg, int aIndex)
{
	...
	if (!gObjJoominCheck(aIndex, lpMsg->LastJoominNumber))
	{
		...
		LogAdd("[CharTrasfer] Fail (JoominNumber) [%s][%s]",
			lpObj->AccountID, lpObj->Name);
	...
	LogAdd("[CharTrasfer] Request Character Trasfer [%s][%s]", lpObj->AccountID, lpObj->Name);
}
...
void CGDuelStartRequestRecv(PMSG_REQ_START_DUEL * lpMsg, int aIndex)
{
	...
	LogAdd("[Duel] [%s][%s] Requested to Start Duel to [%s][%s]", lpObj->AccountID, lpObj->Name, gObj[iDuelIndex].AccountID, gObj[iDuelIndex].Name);
}
...
void CGDuelStartRequestRecv(PMSG_REQ_START_DUEL * lpMsg, int aIndex)
{
	...
	char szDuelName2[MAX_ACCOUNT_LEN + 1];
	BOOL bRetVal;
...
void CGDuelEndRequestRecv(PMSG_REQ_END_DUEL * lpMsg, int aIndex) //Identical
{
	...
	if (OBJMAX_RANGE(gObj[iDuelIndex].m_iDuelUser))
	{
		if (gObj[iDuelIndex].m_iDuelUser == aIndex)
		{
			...
			LogAdd("[Duel] [%s][%s] Duel Ended [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[iDuelIndex].AccountID, gObj[iDuelIndex].Name);
		}
	}
#endif
}

void CGDuelOkRequestRecv(PMSG_ANS_DUEL_OK * lpMsg, int aIndex) //Identical
{
	...
	if (OBJMAX_RANGE(iDuelIndex))
	{
		...
		else if (gObj[iDuelIndex].Type == OBJ_MONSTER)
		{
			return;
		}
		else if (gObj[iDuelIndex].CloseCount >= 0)
		{
			return;
		}
		else if (gObj[iDuelIndex].Custom->InEvent == true || DeathEvent.PlayerStruct[iDuelIndex]._InEvent == true
			|| TheftEvent.PlayerStruct[iDuelIndex]._InEvent == true)
		{
			return;
		}
		...
		if (gObj[iDuelIndex].m_iDuelUserReserved == aIndex)
		{
			...
			if (!strcmp(szDuelName, szDuelName2))
			{
				if (lpMsg->bDuelOK == false)
				{
					...
					gObj[aIndex].Custom->InDuel = false;

					gObj[iDuelIndex].m_iDuelUserReserved = -1;

					gObj[iDuelIndex].Custom->InDuel = false;
				...
				else if (lpMsg->bDuelOK == true)
				{
					...
					gObj[iDuelIndex].Custom->InDuel = true;
					...
					gObj[aIndex].Custom->InDuel = true;
					...
					LogAdd("[Duel] [%s][%s] Duel Started [%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[iDuelIndex].AccountID, gObj[iDuelIndex].Name);
				}
			}
			else
			{
...
void GCSendDuelScore(int aIndex1, int aIndex2) //Identical
{
...
void GCGuildViewportInfo(PMSG_REQ_GUILDVIEWPORT * aRecv, int aIndex)
{
	...
	{
		LogAdd("¡Ú¡Ú¡Ù ±æµå Á¤º¸ Ã£À»¼ö ¾øÀ½. ÀÌ¸§ : [%s] ¹øÈ£ : %d",
...
void CGReqMapSvrAuth(PMSG_REQ_MAPSERVERAUTH * lpMsg, int iIndex)
{
	...
	if (strcmp((char*)Serial, Setting._Serial))
	{
		LogAddC(2, "error-L1: Serial error [%s] [%s]", Account, Serial);
	...
	if (PacketCheckTime(lpObj) == FALSE)
	{
		LogAdd(lMsg.Get(MSGGET(1, 217)), iIndex, Account);
		CloseClient(iIndex);
	...
	{
		LogAdd(lMsg.Get(MSGGET(1, 218)), iIndex, Account);
		CloseClient(iIndex);
	...
	if (gObj[iIndex].LoginMsgSnd != 0)
	{
		...
		{
			LogAdd(lMsg.Get(MSGGET(1, 219)), iIndex, Account, gObj[iIndex].Ip_addr);
		}
		else
		{
	...
	memcpy(gObj[iIndex].AccountID, Account, MAX_ACCOUNT_LEN);
	memset(gObj[iIndex].Name, 0, MAX_ACCOUNT_LEN);
	gObj[iIndex].m_cAccountItemBlock = 0;
	GJReqMapSvrAuth(iIndex, gObj[iIndex].AccountID, lpMsg->Name,
	lpMsg->iJoinAuthCode1, lpMsg->iJoinAuthCode2,
	lpMsg->iJoinAuthCode3, lpMsg->iJoinAuthCode4);

	LogAdd("[MapServerMng] Map Server Join Send : [%s][%s](%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, iIndex);
}
...
void CGReqCastleSiegeState(PMSG_REQ_CASTLESIEGESTATE * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
...
void GCAnsCastleSiegeState(int iIndex, int iResult, LPSTR lpszGuildName, LPSTR lpszGuildMaster) //Identical (HermeX Fix Applied)
{
#if (GS_CASTLE==1)
	...
	SYSTEMTIME tmSiegeStartDate = { 0 }; //HermeX Fix
	...
	// Siege Start Date
	pMsgResult.btSiegeStartYearH = SET_NUMBERH(tmSiegeStartDate.wYear);  //HermeX Fix
	pMsgResult.btSiegeStartYearL = SET_NUMBERL(tmSiegeStartDate.wYear);
	pMsgResult.btSiegeStartMonth = tmSiegeStartDate.wMonth;  //HermeX Fix
	pMsgResult.btSiegeStartDay = tmSiegeStartDate.wDay;  //HermeX Fix
	pMsgResult.btSiegeStartHour = tmSiegeStartDate.wHour;  //HermeX Fix
	pMsgResult.btSiegeStartMinute = tmSiegeStartDate.wMinute;  //HermeX Fix
	// Time Left
...
void CGReqRegCastleSiege(PMSG_REQ_REGCASTLESIEGE * lpMsg, int iIndex) //Identical
{
#if (GS_CASTLE==1)
...
void GCAnsRegCastleSiege(int iIndex, int iResult, LPSTR lpszGuildName) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqGiveUpCastleSiege(PMSG_REQ_GIVEUPCASTLESIEGE * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	if ((g_CastleSiege.GetCastleState() < 1) || (g_CastleSiege.GetCastleState() > 3)) //Fixed
	{
...
void GCAnsGiveUpCastleSiege(int iIndex, int iResult, int bGiveUp, int iMarkCount, LPSTR lpszGuildName) //Changed by HermeX
{
#if(GS_CASTLE==1) //Identical
...
void CGReqGuildRegInfo(PMSG_REQ_GUILDREGINFO * lpMsg, int iIndex) //Identical
{
#if (GS_CASTLE==1)
	if (lpMsg == NULL)
		return;
...
void GCAnsGuildRegInfo(int iIndex, int iResult, CSP_ANS_GUILDREGINFO* lpMsgResult) //Changed by HermeX
{
#if (GS_CASTLE==1) //Identical
...
void CGReqRegGuildMark(PMSG_REQ_REGGUILDMARK * lpMsg, int iIndex) //Identical
{
#if (GS_CASTLE==1)
	if (lpMsg == NULL)
		return;
	...
	if (CHECK_LIMIT(lpMsg->btItemPos, 64) == FALSE) //Fixed 64-1 = 63 (0x3F)
	{
...
void GCAnsRegGuildMark(int iIndex, int iResult, CSP_ANS_GUILDREGMARK* lpMsgResult) //Identical
{
#if (GS_CASTLE==1)
	...
	else
	{
		if (gObj[iIndex].pInventory[lpMsgResult->iItemPos].m_Type != ITEMGET(14, 21) &&
			...
			gObj[iIndex].pInventory[lpMsgResult->iItemPos].m_Durability <= 0.0)
		{
			PMSG_ANS_REGGUILDMARK pMsgResult = { 0 };
			pMsgResult.h.set((LPBYTE)&pMsgResult, 0xB2, 0x04, sizeof(pMsgResult));
			pMsgResult.btResult = 3;
...
void CGReqNpcBuy(PMSG_REQ_NPCBUY * lpMsg, int iIndex) //Identical
{
#if (GS_CASTLE==1)
	if (lpMsg == NULL)
		return;
...
void GCAnsNpcBuy(int iIndex, int iResult, int iNpcNumber, int iNpcIndex) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqNpcRepair(PMSG_REQ_NPCREPAIR * lpMsg, int iIndex) //Identical (Need Check)
{
#if (GS_CASTLE==1)
	if (lpMsg == NULL)
		return;
	...
	else
	{
		...
		{
			if ((gObj[iNpcIndex].MaxLife == gObj[iNpcIndex].Life) || (gObj[iNpcIndex].MaxLife < gObj[iNpcIndex].Life)) //HermeX Fix
			{
				gObj[iNpcIndex].Life = gObj[iNpcIndex].MaxLife; //HermeX Fix
				GCAnsNpcRepair(iIndex, 0, lpMsg->iNpcNumber, lpMsg->iNpcIndex, 0, 0);
				return;
			}
			else
			{
				...
				if (gObj[iIndex].Money < iRepairCost) //HermeX Fixed
				{
...
void GCAnsNpcRepair(int iIndex, int iResult, int iNpcNumber, int iNpcIndex, int iNpcHP, int iNpcMaxHP) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqNpcUpgrade(PMSG_REQ_NPCUPGRADE * lpMsg, int iIndex) //Identical
{
#if (GS_CASTLE==1)
				...
				case 1:
					{
						...
						if ((iNXT_UPLVL >= 1) && (iNXT_UPLVL <= 3) && (iNXT_UPLVL == (iCUR_UPLVL + 1))) //Fixed & Identical
						{
				...
				default:
					{
						...
						LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - UpType doesn't Exist [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d",
							gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
						return;
					}
					break;
				}
	...
	else
	{
		if (gObj[iIndex].Money < iNEED_MONEY)
		{
			...
			LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - Money is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Money:%d, Need Money:%d",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue, iNXT_UPLVL, gObj[iIndex].Money, iNEED_MONEY);
		}
		else
		{
			for (iGEMOFDEFEND_COUNT = 0, x = 0; x < 108; x++)//vai no inventário
			{
				if (gObj[iIndex].pInventory[x].IsItem() == TRUE)
				{
					if (gObj[iIndex].pInventory[x].m_Type == ITEMGET(14, 31)) //jewel of guardian
					{
			...
			if (iGEMOFDEFEND_COUNT < iNEED_GEMOFDEFEND)
			{
				...
				LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - Gem is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Gem:%d, Need Gem:%d",
					gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue, iNXT_UPLVL, iGEMOFDEFEND_COUNT, iNEED_GEMOFDEFEND);
			}
			else
			{
...
void GCAnsNpcUpgrade(int iIndex, int iResult, int iNpcNumber, int iNpcIndex, int iNpcUpType, int iNpcUpValue) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqTaxMoneyInfo(PMSG_REQ_TAXMONEYINFO * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	if (lpMsg == NULL)
		return;
...
void GCAnsTaxMoneyInfo(int iIndex, int iResult, BYTE btTaxRateChaos, BYTE btTaxRateStore, __int64 i64Money) //Identical
{
#if (GS_CASTLE==1)
...
void CGReqTaxRateChange(PMSG_REQ_TAXRATECHANGE * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
...
void GCAnsTaxRateChange(int iIndex, int iResult, BYTE btTaxType, int iTaxRate) //Identical
{
#if (GS_CASTLE==1)
...
void CGReqMoneyDrawOut(PMSG_REQ_MONEYDRAWOUT * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	int iMoneyChange;
	int bRETVAL;
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					GS_GDReqCastleMoneyChange(g_MapServerManager.GetMapSvrGroup(), iIndex, -iMoneyChange); //Fixed
...
void GCAnsMoneyDrawOut(int iIndex, int iResult, __int64 i64Money) //Identical
{
#if(GS_CASTLE==1)
...
void GCAnsCsGateState(int iIndex, int iResult, int iGateIndex) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqCsGateOperate(PMSG_REQ_CSGATEOPERATE * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	int iGateIndex;
...
void GCAnsCsGateOperate(int iIndex, int iResult, int iGateIndex, int iGateOperate) //Identical
{
#if(GS_CASTLE==1)
...
void GCAnsCsGateCurState(int iIndex, int iGateIndex, int iGateOperate) //Identical
{
#if(GS_CASTLE==1)
...
void GCAnsCsAccessSwitchState(int iIndex, int iSwitchIndex, int iSwitchUserIndex, BYTE btSwitchState) //Identical
{
#if(GS_CASTLE==1)
	...
	pMsgResult.btIndex2 = SET_NUMBERL(iSwitchIndex & 0xffff); //Fixed
	pMsgResult.btUserIndex1 = SET_NUMBERH(iSwitchUserIndex & 0xffff);
	pMsgResult.btUserIndex2 = SET_NUMBERL(iSwitchUserIndex & 0xffff); //Fixed
	pMsgResult.btSwitchState = btSwitchState;
	DataSend(iIndex, (LPBYTE)&pMsgResult, pMsgResult.h.size);
#endif
}

void GCAnsCsAccessCrownState(int iIndex, BYTE btCrownState) //Identical
{
#if(GS_CASTLE==1)
	...
	if (btCrownState == 0)
	{
		...
		LogAdd("[CastleSiege] [Reg. Attempt] Crown AccessTime : %d [%s](%s)(%s)", gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID, gObj[iIndex].Name);
	}

	if (btCrownState == 1)
	{
		LogAdd("[CastleSiege] [Reg. Success] Crown AccessTime : %d [%s](%s)(%s)", gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	if (btCrownState == 2)
	{
		...
		gObj[iIndex].m_iAccumulatedCrownAccessTime -= iCrownTime;
		...
		LogAdd("[CastleSiege] [Reg. Fail] Crown AccessTime : %d [%s](%s)(%s)", gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID, gObj[iIndex].Name);
	}
...
void GCAnsCsNotifyStart(int iIndex, BYTE btStartState) //Identical
{
#if(GS_CASTLE==1)
...
void GCAnsCsNotifyProgress(int iIndex, BYTE btCastleSiegeState, LPCSTR lpszGuildName) //Identical
{
	if (lpszGuildName == NULL)
		return;
...
void GCAnsCsMapSvrTaxInfo(int iIndex, BYTE btTaxType, BYTE btTaxRate) //Identical
{
...
void CGReqCsMiniMapData(PMSG_REQ_MINIMAPDATA * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	if (gObjIsConnected(iIndex) == FALSE)
		return;
...
void GCAnsCsMiniMapData(int iIndex, BYTE btResult) //Identical
{
...
void CGReqStopCsMiniMapData(PMSG_REQ_STOPMINIMAPDATA * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	if (gObjIsConnected(iIndex) == FALSE)
		return;
...
void CGReqCsSendCommand(PMSG_REQ_CSCOMMAND * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	if (gObjIsConnected(iIndex) == FALSE)
		return;
...
void GCAnsCsSendCommand(int iCsJoinSize, BYTE btTeam, BYTE btX, BYTE btY, BYTE btCommand) //Identical
{
#if(GS_CASTLE==1)
	PMSG_ANS_CSCOMMAND pMsgResult;

	pMsgResult.h.set((LPBYTE)&pMsgResult, 0xB2, 0x1D, sizeof(pMsgResult)); //#TODO
...
void CGReqCsSetEnterHuntZone(PMSG_REQ_CSHUNTZONEENTER * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	if (gObjIsConnected(iIndex) == FALSE)
		return;
...
void GCAnsCsSetEnterHuntZone(int iIndex, BYTE btResult, BYTE btEnterHuntZone) //Identical
{
...
void CGReqNpcDbList(PMSG_REQ_NPCDBLIST * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqCsRegGuildList(PMSG_REQ_CSREGGUILDLIST * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
...
void CGReqCsAttkGuildList(PMSG_REQ_CSATTKGUILDLIST * lpMsg, int iIndex) //Identical
{
#if(GS_CASTLE==1)
	char cBUFFER[1022];
...
#if(GS_CASTLE == 1)
void GCSendObjectCreationState(int iObjectIndex) //Identical
{
	...
	if (lpObj->Class == 278)
	{
		...
		MsgSendV2(lpObj, (LPBYTE)&pMsg, sizeof(pMsg)); //Fixed
	}
}
#endif
...
#if(GS_CASTLE==1)
void CGReqCastleHuntZoneEntrance(PMSG_REQ_MOVE_TO_CASTLE_HUNTZONE * aRecv, int iIndex) //Identical
{
	if (!OBJMAX_RANGE(iIndex))
		return;
	...
	PHeadSubSetB((LPBYTE)&pMsg, 0xB9, 0x05, sizeof(pMsg)); //Fixed
	...
	DataSend(iIndex, (LPBYTE)&pMsg, sizeof(pMsg)); //Fixed
}
#endif
