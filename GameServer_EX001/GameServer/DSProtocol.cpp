...
void DataServerProtocolCore(BYTE Protocol, LPBYTE Data, int Size)
{
	...
	{
	case 0x00:
		DataServerLoginResult((SDHP_RESULT*)(Data));
		break;
	case 0x01:
		JGPGetCharList(Data);
		break;
	case 0x04:
		JGCharacterCreateRequest((SDHP_CREATECHARRESULT*)(Data));
		break;
	case 0x05:
		JGCharDelRequest((SDHP_CHARDELETERESULT*)(Data));
		break;
	case 0x06:
		JGGetCharacterInfo((SDHP_DBCHAR_INFORESULT*)(Data)); // CharacterInfoRecv
		break;
	case 0x08:
		DGGetWarehouseList((SDHP_GETWAREHOUSEDB_SAVE*)(Data));
		break;
	case 0x10:
		GDGetWarehouseNoItem((SDHP_GETWAREHOUSEDB_RESULT*)(Data));
		break;
	case 0x52:
		ItemSerialCreateRecv((SDHP_ITEMCREATERECV*)(Data));
		break;
	case 0x56:
		DGRecvPetItemInfo(Data);
		break;
	case 0x60:
		DGOptionDataRecv((SDHP_SKILLKEYDATA_SEND*)(Data));
		break;
	case 0x80:
		{
			PMSG_DEFAULT2 * lpDef1 = (PMSG_DEFAULT2*)(Data);

			switch (lpDef1->subcode)
			{
			case 0x00:
				GS_DGAnsCastleTotalInfo(Data);
				break;
			case 0x01:
				GS_DGAnsOwnerGuildMaster(Data);
				break;
			case 0x03:
				GS_DGAnsCastleNpcBuy(Data);
				break;
			case 0x04:
				GS_DGAnsCastleNpcRepair(Data);
				break;
			case 0x05:
				GS_DGAnsCastleNpcUpgrade(Data);
				break;
			case 0x06:
				GS_DGAnsTaxInfo(Data);
				break;
			case 0x07:
				GS_DGAnsTaxRateChange(Data);
				break;
			case 0x08:
				GS_DGAnsCastleMoneyChange(Data);
				break;
			case 0x09:
				GS_DGAnsSiegeDateChange(Data);
				break;
			case 0x0A:
				GS_DGAnsGuildMarkRegInfo(Data);
				break;
			case 0x0B:
				GS_DGAnsSiegeEndedChange(Data);
				break;
			case 0x0C:
				GS_DGAnsCastleOwnerChange(Data);
				break;
			case 0x0D:
				GS_DGAnsRegAttackGuild(Data);
				break;
			case 0x0E:
				GS_DGAnsRestartCastleState(Data);
				break;
			case 0x0F:
				GS_DGAnsMapSvrMsgMultiCast(Data);
				break;
			case 0x10:
				GS_DGAnsRegGuildMark(Data);
				break;
			case 0x11:
				GS_DGAnsGuildMarkReset(Data);
				break;
			case 0x12:
				GS_DGAnsGuildSetGiveUp(Data);
				break;
			case 0x16:
				GS_DGAnsNpcRemove(Data);
				break;
			case 0x17:
				GS_DGAnsCastleStateSync(Data);
				break;
			case 0x18:
				GS_DGAnsCastleTributeMoney(Data);
				break;
			case 0x19:
				GS_DGAnsResetCastleTaxInfo(Data);
				break;
			case 0x1A:
				GS_DGAnsResetSiegeGuildInfo(Data);
				break;
			case 0x1B:
				GS_DGAnsResetRegSiegeInfo(Data);
				break;
			}
		}
		break;
	case 0x81:
		GS_DGAnsCastleInitData(Data);
		break;
	case 0x82:
		GS_DGAnsCastleNpcInfo(Data);
		break;
	case 0x83:
		GS_DGAnsAllGuildMarkRegInfo(Data);
		break;
	case 0x84:
		GS_DGAnsFirstCreateNPC(Data);
		break;
	case 0x85:
		GS_DGAnsCalcREgGuildList(Data);
		break;
	case 0x86:
		GS_DGAnsCsGulidUnionInfo(Data);
		break;
	case 0x87:
		GS_DGAnsCsSaveTotalGuildInfo(Data);
		break;
	case 0x88:
		GS_DGAnsCsLoadTotalGuildInfo(Data);
		break;
	case 0x89:
		GS_DGAnsCastleNpcUpdate(Data);
		break;
	case 0xCF:
		DGMoveOtherServer((SDHP_CHARACTER_TRANSFER_RESULT*)(Data));
		break;
	}
}
...
void JGPGetCharList(unsigned char * lpRecv)
{
	...
	pCLCount.MaxClass = gObj[aIndex].Magumsa + Setting._Character; // Mudar DL AQuI
	...
	if (pCLCount.Count > 0)
	{
		for (int n = 0; n < pCLCount.Count; n++)
		{
			...
			else	// Case DB 3
			{
				...
				if (TempInventory[7] >= 100 && TempInventory[7] <= 200)
				{
					pCList.CharSet[5] |= CS_SET_WING1(3);
					pCList.CharSet[17] |= (BYTE)(TempInventory[7] - 100 + 1) << 2;
				}
			}
			if (TempInventory[8] >= 100 && TempInventory[8] <= 150)
			{
				pCList.CharSet[5] |= CS_SET_HELPER(3);
				pCList.CharSet[16] |= (BYTE)(TempInventory[8] - 100 + 1) << 1;
			}
...
void JGGetCharacterInfo(SDHP_DBCHAR_INFORESULT * lpMsg)
{
	...
	for (int i = OBJ_STARTUSERINDEX; i < OBJMAX; i++)
	{
		if (gObj[i].Connected == PLAYER_PLAYING)
		{
			if (gObj[i].Type == OBJ_USER)
			{
				...
				{
					LogAdd("[Anti-HACK][JGGetCharacterInfo] Attempted Character-Copy by double logging [%s][%s]", szName, gObj[aIndex].AccountID);
	...
	LogAdd("[FRUIT System] [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)", lpObj->AccountID, lpObj->Name, MinusPoint, AddPoint, MaxMinusPoint, MaxAddPoint);

	if (AddPoint < 0 || AddPoint > MaxAddPoint || MinusPoint < 0 || MinusPoint > MaxMinusPoint)
	{
		LogAdd("[FRUIT System] Character Stat Error [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)", lpObj->AccountID, lpObj->Name, MinusPoint, AddPoint, MaxMinusPoint, MaxAddPoint);
	}
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE * lpMsg)
{
	...
	for (int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		...
		if (itype != -1)
		{
			...
			if (dbsize >= 10)
			{
				item.m_Option3 |= DBI_GET_DINO_OPTION(lpMsg->dbItems[n*dbsize + 7]); // Fix Life + 28
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV * lpMsg)
{
	...
	else if (lpMsg->MapNumber == 236)
	{
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED)
		{
			...
			pCreateItem.Convert(lpMsg->Type, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0, 3);
			
			pCreateItem.m_Number = lpMsg->m_Number;

			BYTE btItemPos = Function.gObjInventoryInsertItemEx(lpMsg->aIndex, pCreateItem);

			if (btItemPos == (BYTE)-1)
			{
				LogAdd("Error: Falha ao inserir o item no inventário [%s][%s]", gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			}
			else
			{
...
void GS_DGAnsCastleNpcBuy(unsigned char * lpRecv) //Identical
{
#if(GS_CASTLE==1)
	...
	if (lpMsg->iResult == 1)
	{
		...
		if (bAddResult == TRUE)
		{
			...
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() OK - Npc:(CLS:%d, IDX:%d)", lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
		else
		{
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)", lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
	}
	else
	{
		LogAdd("[CastleSiege] GS_DGAnsCastleNpcBuy() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)", lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
	}
...
void GS_DGAnsCastleNpcRepair(LPBYTE lpRecv) //Identical
{
#if(GS_CASTLE==1)
	...
	if (lpMsg->iResult == 1)
	{
		...
		if (bRepairResult == TRUE)
		{
			...
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() OK - Npc:(CLS:%d, IDX:%d)", lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
		else
		{
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)", lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
	}
	else
	{
		LogAdd("[CastleSiege] GS_DGAnsCastleNpcRepair() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)", lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
	}
...
void GS_DGAnsCastleNpcUpgrade(LPBYTE lpRecv) //Identical
{
#if (GS_CASTLE==1)
	...
	else
	{
		...
		LogAdd("[CastleSiege] [0x80][0x06] GS_DGAnsTaxInfo() - Npc Upgrade OK (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)", lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
	}
...
void GS_DGAnsTaxInfo(LPBYTE lpRecv) //Identical (Fixed 02/12/2009)
{
#if(GS_CASTLE==1)
...
void GS_DGAnsTaxRateChange(LPBYTE lpRecv) //Identical (Fixed 02/12/2009)
{
#if(GS_CASTLE==1)
...
void GS_DGAnsCastleMoneyChange(LPBYTE lpRecv) //Identical
{
#if(GS_CASTLE==1)
	...
	if (lpMsg->iResult == TRUE)
	{
		if (gObjIsConnected(lpMsg->iIndex))
		{
			if (lpMsg->iMoneyChanged < 0)
			{
				...
				LogAdd("[CastleSiege] [0x80][0x08] GS_DGAnsCastleMoneyChange() - Withdraw Request OK [%s][%s] (ReqMoney:%d, TotMoney:%I64d)", gObj[lpMsg->iIndex].AccountID,
...
void GS_DGAnsResetCastleTaxInfo(LPBYTE lpRecv) //Identical
{
#if(GS_CASTLE==1)
...
void GS_DGAnsAllGuildMarkRegInfo(LPBYTE lpRecv) //Identical
{
#if(GS_CASTLE==1)
...
void GS_DGAnsFirstCreateNPC(LPBYTE lpRecv) //Identical
{
#if(GS_CASTLE==1)
...
void GS_DGAnsCalcREgGuildList(LPBYTE lpRecv) //Identical
{
#if (GS_CASTLE==1)
...
void GS_DGAnsCsGulidUnionInfo(LPBYTE lpRecv) //Identical
{
#if (GS_CASTLE==1)
...
void GS_DGAnsCsLoadTotalGuildInfo(LPBYTE lpRecv) //Identical
{
#if (GS_CASTLE==1)
...
void GS_DGAnsCastleNpcUpdate(LPBYTE lpRecv) //Identical
{
	...
	LogAdd("[CastleSiege] [0x89] GS_DGAnsCastleNpcUpdate() - Npc Data Update Result : '%s'", szResult);
#else
	return;
#endif
}