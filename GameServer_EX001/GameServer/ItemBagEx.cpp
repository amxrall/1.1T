...
BOOL CItemBagEx::DropKundunEventItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	...
	if (this->GetBagCount() > 0)
	{
		if (true)
		{
			...
			LogAdd("[¡ÛKundun EVENT] ¡ÚKUNDUN Event ItemDrop [%s][%s] [%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d", lpObj->AccountID, lpObj->Name, this->m_sEventName, btMapNumber, x, y, ItemAttribute[type].Name, type, level, Option1, Option2, Option3);
...
void CItemBagEx::DropBloodCastleItem(int aIndex)
{
	if (this->m_bLoad == FALSE)
	{
		return;
	...
	if (this->GetBagCount() > 0)
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, iType, iLevel, (BYTE)fDur, iOption1, iOption2, iOption3, lpObj->m_Index, iExOption, 0);

		LogAdd("[ BLOODCASTLE ][ Reward ] Reward Item Drop [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
			lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, ItemAttribute[iType].Name,
			iType, iLevel, iOption1, iOption2, iOption3, iExOption);
	}
	else
	{
		ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, ITEMGET(12, 15), 0, 0, 0, 0, 0, lpObj->m_Index, 0, 0);
	}
}

void CItemBagEx::DropWhiteWizardItem(int aIndex, int LootIndex)
{
	if (this->m_bLoad == FALSE)
	{
		return;
	...
	if (this->GetBagCount() > 0)
	{
		...
		ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, iType, iLevel, (BYTE)fDur, iOption1, iOption2, iOption3, LootIndex, iExOption, 0);

		LogAdd("[ WHITEWIZARD ][ Reward ] Reward Item Drop [%s][%s] : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d ExOp:%d",
			lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, ItemAttribute[iType].Name,
			iType, iLevel, iOption1, iOption2, iOption3, iExOption);
	}
	else
	{
		ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, ITEMGET(14, 13), 0, 0, 0, 0, 0, lpObj->m_Index, 0, 0);
	}
}