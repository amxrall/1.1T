...
{
	LogAdd("[DevilSquare] 이벤트 경품 요청 [%s][%s]", AccountId, Name);	// #warning Translation
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		if (lpObj->pChaosBox[n].IsItem() == TRUE)
		{
			...
			LogAdd("[DevilSquare,%d] [%s][%s] CBMix [%d,%s,%d,%d,%d,%d]serial:[%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380Ex[%d] HO:[%d,%d]",
				...
				lpObj->pChaosBox[n].m_SetOption, 0, 0, 0);
		}
	}
}
...
BOOL PlusItemLevelChaosMix(LPOBJ lpObj, int mixType)
{
	...
	{
		lpObj->ChaosSuccessRate = Setting._Mix2[lpObj->Custom->VipCount];
	}
	else if (lpObj->pChaosBox[PlusItemPos].m_Level == 10)
	{
		lpObj->ChaosSuccessRate = Setting._Mix3[lpObj->Custom->VipCount];
	}
	else if (lpObj->pChaosBox[PlusItemPos].m_Level == 11)
	{
		lpObj->ChaosSuccessRate = Setting._Mix4[lpObj->Custom->VipCount];
	}
	else if (lpObj->pChaosBox[PlusItemPos].m_Level == 12)
	{
		lpObj->ChaosSuccessRate = Setting._Mix5[lpObj->Custom->VipCount];
	}
	...
	if (lpObj->ChaosSuccessRate > Setting._Mix1[3])
	{
		lpObj->ChaosSuccessRate = Setting._Mix1[3];
	}
...
BOOL PegasiaChaosMix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = Setting._Mix1[2];	// Succes Rate for Dinorant
	lpObj->ChaosSuccessRate += iCharmOfLuckCount;
	lpObj->Money -= nChaosNeedMoney;
...
BOOL WingChaosMix(LPOBJ lpObj)
{
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		if (lpObj->pChaosBox[n].IsItem() == TRUE)
		{
			...
			else if (lpObj->pChaosBox[n].m_Type >= ITEMGET(12, 0) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12, 2))
			{
				WingCount++;
				WingIndex = n;
	...
	if (iSleeveOfLord == 1)
	{
		lpObj->ChaosSuccessRate = Setting._Mix1[0];
	}
	else if (LokesFeathersCount == 1)
	{
		lpObj->ChaosSuccessRate = Setting._Mix1[1];
	}
	...
	if ((rand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		::gObjInventoryCommit(lpObj->m_Index);
		::LogAdd("[WingMix,2] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->Money, nChaosNeedMoney, iCharmOfLuckCount);
		return TRUE;
	}
	else
	{
		...
		GCUserChaosBoxSend(lpObj, 0);
		DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		::LogAdd("[WingMix,2] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->Money, nChaosNeedMoney, iCharmOfLuckCount);
...
//GS-CS Here
#if(GS_CASTLE==1)
void CastleSpecialItemMix(LPOBJ lpObj) //Identical
{
	lpObj->ChaosLock = TRUE;
	...
	{
		LogAdd("[CastleSpecialMix] [%s][%s] Item Mix Failed - Item Error (DG:%d, BGx10:%d, SGx10:%d, Other:%d)",
	...
	LogAdd("[CastleSpecialMix] [%s][%s] Chaos Mix Start", lpObj->AccountID, lpObj->Name);
