...
void CEventManagement::Run()
{
	...
	{
		for (auto it = this->m_vtEventTime.begin(); it != this->m_vtEventTime.end(); it++)
		{
			this->m_bEventStarted[it->m_eEventKind] = false;
		}

		LogAddTD("[Event Management] ³¯Â¥ º¯°æµÊ. %02d %02d %02d", this->m_wToday_Year, this->m_wToday_Month, this->m_wToday_Day);
	...
	for (auto it = this->m_vtEventTime.begin(); it != this->m_vtEventTime.end(); it++)
	{
		...
		{
			if (this->m_bEventStarted[it->m_eEventKind])
			{
				LogAddTD("[Event Management] End: (%d) %02d %02d", it->m_eEventKind, it->m_iHour, sysTime.wMinute);

				this->m_bEventStarted[it->m_eEventKind] = false;
				this->EndEvent(it->m_eEventKind);
			}
			else
			{
				LogAddTD("[Event Management] Start: (%d) %02d %02d", it->m_eEventKind, it->m_iHour, sysTime.wMinute);

				this->m_bEventStarted[it->m_eEventKind] = true;
...
void CEventManagement::EndEvent(int eEventKind)
{
	switch (eEventKind)
	{
	case EVENT_ID_DRAGONHERD:
	...
	while (true)
	{
		...
		while (true)
		{
			if (Type == 0)
			{
				...
				this->m_bEventStarted[EIT.m_eEventKind] = false;
