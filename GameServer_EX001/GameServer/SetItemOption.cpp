...
BOOL CSetItemOption::LoadOptionInfo(LPSTR filename)
{
	...
	while (true)
	{
		...
		else
		{
			...
			Token = GetToken();
			p->RequireClass[WIZARD] = TokenNumber;

			Token = GetToken();
			p->RequireClass[KNIGHT] = TokenNumber;

			Token = GetToken();
			p->RequireClass[ELF] = TokenNumber;

			Token = GetToken();
			p->RequireClass[GLADIATOR] = TokenNumber;

			Token = GetToken();
			p->RequireClass[LORD] = TokenNumber;
