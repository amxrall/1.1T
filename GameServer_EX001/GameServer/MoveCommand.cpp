...
int CMoveCommand::Load(char* filename)
{
	...
	while (true)
	{
		...
		if (Token == 1)
		{
			...
			LogAdd("[LoadMoveReq]: %d %s %s %d %d %d", this->m_MoveCommandData[counter].Index,
				this->m_MoveCommandData[counter].Name,
				this->m_MoveCommandData[counter].EngName,
				this->m_MoveCommandData[counter].NeedZen,
				this->m_MoveCommandData[counter].NeedLevel,
				this->m_MoveCommandData[counter].GateNumber);
...
BOOL CMoveCommand::Move(LPOBJ lpObj, LPSTR mapname)
{
	...
	if (GateNumber == 119)
	{
		if (lpObj->Custom->VipCount == 0)
		{
			Function.MsgOutput(lpObj->m_Index, 1, "Exclusivo para jogadores vip's.");
			return FALSE;
		}
	}
	...
	LogAdd("[%s][%s] Use [%s %s] Fail GateNumber = %d", lpObj->AccountID, lpObj->Name, lMsg.Get(MSGGET(11, 185)), mapname, GateNumber);
