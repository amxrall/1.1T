...
void EledoradoBoxOpenEven(LPOBJ lpObj, int boxtype, int addlevel, int money)
{
	...
	if (EledoradoBox->GetBagCount() > 0)
	{
		if ((rand() % 100) < ItemDropRate)
		{
			...
			LogAdd("[%s][%s][Lucky Box Item Drop][Eledorado Box] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, NewItem.GetName(), type, level, Option1, Option2, Option3);
...
void HeartOfLoveOpenEven(LPOBJ lpObj)
{
	...
	if (HeartOfLoveItemBag.GetBagCount() > 0)
	{
		if ((rand() % 20) < g_ItemDropRateForgHeartOfLove)
		{
			...
			LogAdd("[%s][%s][Lucky Box Item Drop][Heart Of Love] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(), type, level, Option1, Option2, Option3);
...
void EGResultRegEventChip(PMSG_ANS_REGISTER_EVENTCHIP * aRecv)
{
	...
	{
		Result.ChipCount = -1;
		LogAdd("[EventChip] [%s][%s] RegEventServer Fail (RegEventchip) %d", lpObj->AccountID, lpObj->Name, aRecv->Pos);
	}
	else
	{
		...
		LogAdd("[EventChip] [%s][%s] Delete EventChip (%d)", lpObj->AccountID, lpObj->Name, aRecv->Pos);

		//ADD Premiação aqui para o GA
	}
...
void EGRecvRegStone(PMSG_ANS_REGISTER_STONES* aRecv)
{
	...
	if (aRecv->bSUCCESS != FALSE)
	{
		...
		LogAdd("[Stone] [%s][%s] Delete Stones", lpObj->AccountID, lpObj->Name);
	}
	else
	{
		Result.ChipCount = -1;
		LogAdd("[Stone] [%s][%s] RegEventServer Fail (Stones : %d)", lpObj->AccountID, lpObj->Name, aRecv->iStoneCount);
	}
...
void EGRecvChangeStones(PMSG_ANS_RESET_EVENTCHIP* aRecv)
{
	...
	if (aRecv->bSUCCESS != FALSE)
	{
		...
		LogAdd("[Stones] [%s][%s] ChangeRena AddMoney(%d)", lpObj->AccountID, lpObj->Name, lpObj->iStoneCount * 3000);
	}
	else
	{
...
void EGRecv2AnvRegSerial(PMSG_ANS_2ANIV_SERIAL* aRecv)
{
	...
	if (aRecv->btIsRegistered == FALSE)
	{
		...
		case 11:
			...
			if (g_bRingEventItemTextLoad == FALSE)
			{
				strcpy(Result.szGIFT_NAME, ItemAttribute[ITEMGET(12, 19)].Name);
			}
			break;
		case 12:
			ItemSerialCreateSend(gObj[aRecv->iINDEX].m_Index, GIFT_2ANV_MAP,
