...
//CLogToFile BOT_STORE_LOG("BOT_STORE_LOG", ".\\Log\\BOT_STORE_LOG", 1);
...
void ObjBotStore::Read(char * FilePath)
{
	...
	//BOT_STORE_LOG.Output("[BotStore] - %s file is Loaded");
	fclose(SMDFile);
}
...
void ObjBotStore::AddItem(int bIndex, int botNum)
{
	for (int i = 0; i < this->bot[botNum].ItemCount; i++)
	{
		...
		if (blank != 255)
		{
			...
			item.Convert(this->bot[botNum].storeItem[i].num, item.m_Option1, item.m_Option2, item.m_Option3, item.m_NewOption, item.m_SetOption, 0, 3);
				...
				if (result >= 0)
				{
					...
					for (int i = 0; i < 9; i++)
					{
						if (this->bot[botNum].body[i].num >= 0 && this->bot[botNum].body[i].Enabled == true)
						{
							...
							item.Convert(this->bot[botNum].body[i].num, item.m_Option1, item.m_Option2, item.m_Option3, 0, 0, 0, 3);
