...
static FILE* logfp;
static char m_szLogFileName[100];

CRITICAL_SECTION LogCritical;

int LogMDay, LogMonth, LogMYear, m_cline, gLogOutType = 1;
...
void LogTextPaintProc(HWND hWnd)
{
	...
	HBRUSH Background = CreateSolidBrush(RGB(71, 79, 92)); // FUNDO BRANCO
	//HBRUSH Background1 = CreateSolidBrush(RGB(255, 255, 255)); // FUNDO BRANCO
	...
	Rect1.top = 550; // Cima
	Rect1.left = 1200; // esquerda
	Rect1.right = 1350; // direia
	Rect1.bottom = 600; // baixo
	...
	while (Total-- != 0)
	{
		...
		if (strlen(LogText[i]) > 1)
		{
			SetTextColor(New, RGB((rand() % 255), (rand() % 255), (rand() % 255)));
			TextOut(New, 4, Total * 15 + 227, ":::", 3);
			TextOut(Hdc, 22, Total * 15 + 227, LogText[i], strlen(LogText[i]));
		}
...
void MsgBox(char *szlog, ...)
{
	...
	vsprintf(szBuffer, szlog, pArguments);
	va_end(pArguments);
	MessageBox(NULL, szBuffer, TITLE, MB_OK | MB_APPLMODAL);
}