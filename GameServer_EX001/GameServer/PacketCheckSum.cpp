...
{
	for (int i = 0; i < MAX_PACKET_CHECKSUM_Func_INDEX; i++)
	{
		if (this->PaketChecksum[aIndex].Check[i] == 0)
		{
			LogAdd("PacketCheckSum Error [%d][%s][%s] %d", aIndex, gObj[aIndex].AccountID, gObj[aIndex].Name, i);
...
BOOL CPacketCheckSum::Add(int aIndex, int funcindex, DWORD checksum)
{
	...
	{
		LogAdd("PacketCheckSum : Compare Fail : [%d][%s][%s] (%d,%d)", aIndex, gObj[aIndex].AccountID, gObj[aIndex].Name, funcindex, checksum);
...
{
	if (funcindex < 0 || funcindex >= MAX_PACKET_CHECKSUM_Func_INDEX)
	{
