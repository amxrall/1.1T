...
#if(USE_REG_SYSTEM_IPGET == 1)
		InternetAddr.sin_addr.S_un.S_addr = GetLongHostAddress();
#else
		...
		else
		{
			while (true)
			{
				...
				if (Accept == -1)
				{
					...
					LogAddC(2, "WSAAccept() failed with error %d", WSAGetLastError());
				...
				if (ClientIndex == -1)
				{
					LogAddC(2, "error-L2 : ClientIndex = -1");
					closesocket(Accept);
					LeaveCriticalSection(&criti);
					continue;
				}
				...
				{
					LogAddC(2, "error-L1 : %d %d CreateIoCompletionPort failed with error %d", Accept, ClientIndex, GetLastError());
					closesocket(Accept);
					LeaveCriticalSection(&criti);
					continue;
				}
				...
				{
					LogAddC(2, "error-L1 : %d %d gObjAdd() failed with error %d", Accept, ClientIndex, GetLastError());
					LeaveCriticalSection(&criti);
					closesocket(Accept);
					continue;
				}
				...
				if (nRet == -1)
				{
					...
					{
						LogAddC(2, "error-L1 : WSARecv() failed with error %d", WSAGetLastError());
		...
		if (size <= lpIOContext->nSentBytes)
		{
			...
			if (xcode == 0xC3)
			{
				...
				else
				{
					...
					{
						LogAddC(2, "error-L1 : CStreamPacketEngine Adding Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d", gObj[uIndex].Ip_addr, gObj[uIndex].AccountID, gObj[uIndex].Name, headcode, __FILE__, __LINE__, gObj[uIndex].Connected);
						return 0;
					}

					if (PacketStream.ExtractPacket(byDec) != 0)
					{
						LogAddC(2, "error-L1 : CStreamPacketEngine ExtractPacket Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d", gObj[uIndex].Ip_addr, gObj[uIndex].AccountID, gObj[uIndex].Name, headcode, __FILE__, __LINE__, gObj[uIndex].Connected);
						return 0;
					}
