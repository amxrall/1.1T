...
int CLifeStone::CreateLifeStone(int iIndex) //Identical
{
	...
	if (g_CastleSiegeSync.GetCastleState() != 7) //HermeX Fix
	{
		MsgOutput(iIndex, lMsg.Get(1645));
		return FALSE;
	}

	if (lpObj->GuildStatus != 128) //HermeX Fix
	{
		return FALSE;
	}
	...
	if (iMonsterIndex >= 0)
	{
		...
		LogAdd("[CastleSiege] LifeStone is created - [%s] [%s][%s] (Map:%d)(X:%d, Y:%d)", lpObj->lpGuild->Name,
