...
void MoveMonsterProc() //Identical
{
	DWORD MoveTime;
	int DelayTime;
	LPOBJ lpObj;
...
#else
				if (ATTRIBUTE_RANGE(lpObj->m_Attribute) != FALSE || lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 278 || lpObj->Class == 216 || lpObj->Class == 217 || lpObj->Class == 218 || lpObj->Class == 219) //SirMaster Decompilation
				{
					CreateFrustrum(lpObj->X, lpObj->Y, n);
					continue;
				}
...
void gObjInit()
{
	...
	for (int n = 0; n < OBJMAX; n++)
	{
		...
		if (n >= OBJ_STARTUSERINDEX)
		{
			...
			gObj[n].Custom = new CUSTOMSTRUCT;
		}
		else
		{
...
void gObjEnd()
{
	...
	for (int n = 0; n < OBJMAX; n++)
	{
		...
		if (n >= OBJ_STARTUSERINDEX)
		{
			if (gObj[n].Inventory1)
			{
				delete[] gObj[n].Inventory1;
			}

			if (gObj[n].Inventory2)
			{
				delete[] gObj[n].Inventory2;
			}

			if (gObj[n].InventoryMap1)
			{
				delete[] gObj[n].InventoryMap1;
			}

			if (gObj[n].InventoryMap2)
			{
				delete[] gObj[n].InventoryMap2;
			}

			if (gObj[n].PerSocketContext)
			{
			...
			if (gObj[n].pWarehouse)
			{
				delete[] gObj[n].pWarehouse;
			}

			if (gObj[n].pWarehouseMap)
			{
...
void gObjCloseSet(int aIndex, int Flag)
{
	...
	Function.UpdateRank(aIndex);

	if (lpObj->Type == OBJ_USER)
	{
		if (lpObj->Custom->Hidden == true)
		{
			lpObj->Custom->Hidden = false;
		}
	}
...
void gObjCharZeroSet(int aIndex) //Identical
{
	int i;
	LPOBJ lpObj = &gObj[aIndex];
	int n;
...
void DbItemSetInByte(LPOBJ lpObj, struct SDHP_DBCHAR_INFORESULT* lpMsg, int ItemDbByte, bool* bAllItemExist)
{
	...
	for (n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		if (itype != -1)
		{
			...
			if (ItemDbByte >= 10)
			{
				item.m_Option3 |= DBI_GET_OPTION16(lpMsg->dbInventory[n*ItemDbByte + 7]) >> 4; // Fix to Up to +28
			}
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex) //Identical
{
	int itype;
	int n;
	...
	{
		if (gObj[aIndex].MapNumber == MAP_INDEX_CASTLESIEGE || gObj[aIndex].MapNumber == MAP_INDEX_CASTLEHUNTZONE)
		{
			LogAdd("[ CASTLESIEGE ][ Forced Move From CastleSiegeMap to Lorencia ] (%s)(%s) (BeforeMap:%d(%d/%d))",
	...
	if (lpObj->Class == 0 || lpObj->Class == 2 || lpObj->Class == 1 || lpObj->Class == 4 || lpObj->Class == 3)
	{
		...
		else if (lpObj->Class == 4)	// Dark Lord
		{
			...
			gObjMagicAdd(lpObj, 75, 0);	// Sacrifice
		}
	...
	Connect.PlayerConnect(lpObj->m_Index);

	Attack.Connect(lpObj->m_Index);

	Speed.Connect(lpObj->m_Index);

	Quest.Connect(lpObj->m_Index);
...
BOOL gObjSetMonster(int aIndex, int MonsterClass) //Identical
{
	int Level;
	LPOBJ lpObj;
	...
	{
		//DevGamesX 18/11/2018 - BotSystem
		if (lpObj->m_AttackType >= 100 && lpObj->IsBot == 0)
		{
			// DevGamesX - TEST
...
BOOL gObjGameClose(int aIndex)
{
	...
	{
		if (lpObj->bIsChaosMixCompleted == 1 || lpObj->m_bIsCastleNPCUpgradeCompleted == 1)
		{
...
short gObjDel(int index)
{
	...
	if (Attack.PlayerStruct[lpObj->m_Index]._OffAttack == true)
	{
		return TRUE;
	}
	...
	if (lpObj->Type == OBJ_USER)
	{
		...
		if (UserConnectState >= PLAYER_LOGGED)
		{
			...
			{
				LogAdd("[MapServerMng] User Quit without JoinServer logout-send [%s][%s]", lpObj->AccountID, lpObj->Name);
			}
...
#if(GS_CASTLE == 0)
	DuelEvent.Quit(&gObj[index]);
	PegaEvent.Quit(&gObj[index]);
	SobreEvent.Quit(&gObj[index]);
	DeathEvent.Quit(&gObj[index]);
	TheftEvent.Quit(&gObj[index]);
	CoinEvent.Quit(&gObj[index]);
	Duel.Quit(&gObj[index]);
	Quest.Quit(&gObj[index]);
#endif
...
BOOL gObjCheckXYMapTile(LPOBJ lpObj, int iDbgName)
{
	...
	if (lpObj->Class == 2 && lpObj->Level < 2)
	{
	...
	if ((attr & 4) == 4 || (attr & 8) == 8)
	{
		...
		{
			LogAdd("[ CHECK POSITION ] DbgName[%d] [%s][%s] Map[%d]-(%d,%d) Invalid location causes to force to move", iDbgName, lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
			PMSG_TELEPORT pMsg;
			pMsg.MoveNumber = 17;
			gObjMoveGate(lpObj->m_Index, pMsg.MoveNumber);
...
BOOL gObjBackSpring(LPOBJ lpObj, LPOBJ lpTargetObj) //Identical
{
	int tdir;
	int x;
	int y;
...
BOOL BackSpringCheck(int & x, int & y, int & dir, BYTE map) //Identical
{
	BYTE attr;
	int tx = x;
	int ty = y;
...
BOOL gObjBackSpring2(LPOBJ lpObj, LPOBJ lpTargetObj, int count) //Identical
{
#if(GS_CASTLE==1)
	int tdir;
#endif
...
bool gObjLevelUp(LPOBJ lpObj, int addexp, int iMonsterType, int iEventType)
{
	...
	if (lpObj->Level >= 400 && lpObj->Experience >= gLevelExperience[lpObj->Level - 1])
	{
	...
	if (lpObj->Class == 4)
	{
		lpObj->LevelUpPoint += Setting._Points[2];
	}
	else if (lpObj->Class == 3)
	{
		lpObj->LevelUpPoint += Setting._Points[1];
	}
	else
	{
		lpObj->LevelUpPoint += Setting._Points[0];
	}

	if (lpObj->PlusStatQuestClear != false)
	{
		lpObj->LevelUpPoint++;
		LogAdd("[%s][%s] LevelUp PlusStatQuest Clear AddStat %d", lpObj->AccountID, lpObj->Name, lpObj->LevelUpPoint);
	}
	...
	Function.SendLevelUp(lpObj->m_Index);
...
BOOL gObjLevelUpPointAdd(BYTE Type, LPOBJ lpObj)
{
	...
	case 0:
	{
		if ((lpObj->Strength + 1) <= Setting._MaxStatus)
		{
			lpObj->Strength++;
	...
	case 1:
	{
		if ((lpObj->Dexterity + 1) <= Setting._MaxStatus)
		{
			lpObj->Dexterity++;
	...
	case 2:
	{
		if ((lpObj->Vitality + 1) <= Setting._MaxStatus)
		{
			lpObj->Vitality++;
	...
	case 3:
	{
		if ((lpObj->Energy + 1) <= Setting._MaxStatus)
		{
			lpObj->Energy++;
	...
	case 4:
	{
		if ((lpObj->Leadership + 1) <= Setting._MaxStatus)
		{
			lpObj->Leadership++;
...
void gObjGiveItemSearch(LPOBJ lpObj, int maxlevel) //Empty Caution!
{
}
...
BOOL gObjAttackQ(LPOBJ lpObj) //Identical
{
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj) //Identical
{
	if (lpObj->Type != OBJ_USER)
	{
		return;
	...
	if (gObjGetRelationShip(lpObj, lpTargetObj) == 2)
	{
		...
		{
			LogAdd("[U.System][Rival][Player Kill][¡ÚERROR : Can't find GuildInfo] (  [%s][%s] ) vs ( [%s][%s] )",
...
{
#if(GS_CASTLE == 0)
	Function.PlayerOut(lpObj);

	CoinEvent.Run(lpObj);

	Duel.InvitationCheck(lpObj);

	if (Setting._Reset > 0 && lpObj->Authority == 1)
	{
		if (lpObj->Custom->ResetCount > Setting._Reset || lpObj->Custom->MasterCount >= 1)
		{
			Function.MsgOutput(lpObj->m_Index, 0, "Sala exclusiva para até %d resets!", Setting._Reset);
...
BOOL gObjWingSprite(LPOBJ lpObj)
{
	...
	if ((Wing->m_Type >= ITEMGET(12, 0) && Wing->m_Type <= ITEMGET(12, 6)) || Wing->m_Type <= ITEMGET(12, 5) || NEWWINGS(Wing->m_Type))
	{
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	if (sprite->m_Type == ITEMGET(13, 0)) // angel
	{
		...
		fdamage /= fN;
		if (Setting._PetAngel == 0)
		{
			sprite->m_Durability -= fdamage;
		}
		else
		{
			sprite->m_Durability = 255;
		}
		send_dur = 1;
	}
	else if (sprite->m_Type == ITEMGET(13, 1)) // imp
	{
		fdamage = (damage * 2) / 10.0f;
		fdamage /= fN;
		if (Setting._PetImp == 0)
		{
			sprite->m_Durability -= fdamage;
		}
		else
	...
	if (sprite->m_Type == ITEMGET(13, 100) <= ITEMGET(13, 150)) // angel
	{
		fdamage = (damage * 3) / 10.0f;
		fdamage /= fN;
		if (Setting._NewPets == 0)
		{
			sprite->m_Durability -= fdamage;
		}
		else
	...
	else if (sprite->m_Type == ITEMGET(13, 4))
	{
		...
		if (sprite->m_DurabilitySmall > 1500)
		{
			...
			if (sprite->m_Durability < 1.0f)
			{
				...
				LogAdd("[%s][%s] ´ÙÅ©È£½ºÀÇ»Ô Item is Broken because durability is exhausted [%d]",
...
int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp)
{
	...
	{
		if (lpObj->Custom->VipCount == Reset.ResetStruct[i]._VipCount)
		{
			if (lpObj->Custom->ResetCount >= Reset.ResetStruct[i]._ResetsMin && lpObj->Custom->ResetCount <= Reset.ResetStruct[i]._ResetsMax || Reset.ResetStruct[i]._ResetsMax == -1)
			{
				exp = int(exp * Reset.ResetStruct[i]._Experience);
			}
		}
	}
...
void gObjExpParty(LPOBJ lpObj, LPOBJ lpTargetObj, int AttackDamage, int MSBFlag)
{
		...
		if (number >= 0)
		{
			...
			if (lpTargetObj->MapNumber == lpPartyObj->MapNumber)
			{
				...
				if (dis[n] < 10)
				{
					...
					if (toplevel >= (lpPartyObj->Level + 400))
					{
						totallevel += lpPartyObj->Level + 400;
					}
					else
					{
	...
	if (viewplayer > 1)
	{
		if (bApplaySetParty != 0)
		{
			if (viewplayer == 3)
			{
				viewpercent = Setting._Party[4];
			}
			else if (viewplayer == 4)
			{
				viewpercent = Setting._Party[5];
			}
			else if (viewplayer >= 5)
			{
				viewpercent = Setting._Party[6];
			}
			else
			{
				viewpercent = 120;
		...
		else
		{
			if (viewplayer == 2)
			{
				viewpercent = Setting._Party[0];
			}
			else if (viewplayer == 3)
			{
				viewpercent = Setting._Party[1];
			}
			else if (viewplayer == 4)
			{
				viewpercent = Setting._Party[2];
			}
			else if (viewplayer >= 5)
			{
				viewpercent = Setting._Party[3];
			}
			else
			{
				viewpercent = 150;
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, BYTE Skill) //Identical
{
	if (lpObj->Connected < PLAYER_PLAYING)
	{
		return;
	...
	if (BossEvent._Active == true)
	{
		...
		{
			if (lpTargetObj->Type == OBJ_MONSTER && lpTargetObj->Class == BossEvent._Monster && lpTargetObj->m_Index == BossEvent._Index)
			{
	...
	if (lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
		...
		if (gObjDuelCheck(lpCallObj, lpTargetObj))
		{
			...
			if (lpObj->m_btDuelScore == 5)
			{
				...
				int Color = _ARGB(255, 255, 200, 80); char Buffer[255];

				sprintf_s(Buffer, "[Duel] %s Score: %d vs %s Score: %d, %s Winner!", lpObj->Name, lpObj->m_btDuelScore, lpTargetObj->Name, lpTargetObj->m_btDuelScore, lpObj->Name);

				TNotice::MakeNoticeMsg(&Result, 10, Buffer);
				TNotice::SetNoticeProperty(&Result, 10, Color, 1, 0, 20);
...
void gObjInventoryItemBoxSet(int aIndex, int itempos, int xl, int yl, BYTE set_byte)
{
	...
	for (int y = 0; y < yl; y++)
	{
		...
		for (int x = 0; x < xl; x++)
		{
			...
			//DevGamesX 18/11/2018 - BotSystem
			if (ExtentCheck(xx, yy, 8, 12) == 1 || (gObj[aIndex].IsBot >= 1))
			{
...
BYTE gObjInventoryMoveItem(int aIndex, unsigned char source, unsigned char target, int& durSsend, int& durTsend, unsigned char sFlag, unsigned char tFlag, unsigned char* siteminfo) //Identical
{
	LPOBJ lpObj;
	...
	__try
	{
		...
		if (titem->IsItem() == 1)
		{
			...
			/*
			#if(GS_CASTLE==1) //GameServer 1.00.19 Only
						if(sitem->m_Type == ITEMGET(14,46) || sitem->m_Type == ITEMGET(14,64) )
						{
...
BOOL TradeItemInventoryPutTest(int aIndex)
{
	...
	//DevGamesX 18/11/2018 - BotSystem
	if ((gObj[number].IsBot == 0) && gObjIsConnected(number) == 0)
	{
		return false;
	}
...
void gObjMakePreviewCharSet(int aIndex)
{
	if (!OBJMAX_RANGE(aIndex))
	{
		LogAdd("[MAKE PREVIEW CHARSET] Error : %s %d", __FILE__, __LINE__);
		return;
	}
	// ---
	BYTE index;
	...
	// ---
	if ((lpObj->pInventory[7].m_Type >= ITEMGET(12, 100) && lpObj->pInventory[7].m_Type <= ITEMGET(12, 200)))
	{
	...
	// ---
	if ((lpObj->pInventory[8].m_Type >= ITEMGET(13, 100) && lpObj->pInventory[8].m_Type <= ITEMGET(13, 150)))
	{
		lpObj->CharSet[5] |= CS_SET_HELPER(3);
		lpObj->CharSet[16] |= ((lpObj->pInventory[8].m_Type - 100) + 1 << 1);
	}
}
...
void gObjViewportListCreate(short aIndex) //Identical
{
	int result, n;
	LPOBJ lpObj;
	int mapnum;
...
#else
	else if (lpObj->Type == 2 || lpObj->Type == 3 || lpObj->Type == 3)
	{
		for (n = OBJ_MAXMONSTER; n < OBJMAX; n++)
		{
			lpTempObj = &gObj[n];
...
void PShop_ViewportListRegenarate(short aIndex)
{
	...
	{
		//DevGamesX 18/11/2018 - BotSystem
		if (lpObj->VpPlayer[n].state == 2 && (lpObj->VpPlayer[n].type == OBJ_USER || gObj[lpObj->VpPlayer[n].number].IsBot == 4) && gObj[lpObj->VpPlayer[n].number].m_bPShopOpen == 1)
		{
	...
	if (gObj[aIndex].m_bPShopWantDeal == 1)
	{
		...
		{
			//DevGamesX 18/11/2018 - BotSystem
			if (gObj[gObj[aIndex].m_iPShopDealerIndex].Type == OBJ_USER || gObj[gObj[aIndex].m_iPShopDealerIndex].IsBot == 4)
			{
				...
				else if (gObj[gObj[aIndex].m_iPShopDealerIndex].m_bPShopItemChange == 1 && gObj[gObj[aIndex].m_iPShopDealerIndex].IsBot != 4)
				{
...
#if(GS_CASTLE==1)
			if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE) //HermeX Fix
			{
				if (g_CastleSiege.GetCastleState() == 7)
				{
				...
				if (State != 0)
				{
					...
					if (lpObj->Type == OBJ_USER)
					{
						...
						else if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE)
						{
							...
							if (!g_CsNPC_LifeStone.SetReSpawnUserXY(lpObj->m_Index)) //HermeX Fix
							{
								mgt = 100;
...
int LastTimeRecalculateStats = 0;
...
void gObjSecondProc()
{
	...
	for (int n = 0; n < OBJMAX; n++)
	{
		...
		if (lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if (lpObj->Connected == PLAYER_PLAYING)
			{
				if (lpObj->Type == OBJ_USER)
				{
					...
					{
						GCInventoryItemDeleteSend(n, 8, 1);
					}
				}
			}
			...
			if (lpObj->Type == OBJ_NPC)
			{
				...
				if (CASTLE_CROWNSWITCH_CLASS_RANGE(lpObj->Class)) //HermeX Fix
				{
		...
		if (lpObj->Connected == PLAYER_CONNECTED || lpObj->Connected == PLAYER_LOGGED || lpObj->Connected == PLAYER_PLAYING)
		{
			if (lpObj->Type == OBJ_USER)
			{
				if (lpObj->Connected >= PLAYER_LOGGED && Attack.PlayerStruct[lpObj->m_Index]._OffAttack == false)
				{
		...
		if (lpObj->Type == OBJ_USER && lpObj->Connected != PLAYER_EMPTY)
		{
			TotalPlayer++;
		}
	}
...
int  gObjCurMoveMake(BYTE * const path, LPOBJ lpObj) //Original Source Obtained from GS 0.78.02T WEBZEN
{
	...
	path[0] = (lpObj->Dir << 4);			// ¹æÇâ
	path[0] |= (BYTE)((total - cur) & 0x0f);		// ÀÚ½ÅÀÇ À§Ä¡ Æ÷ÇÔ °³¼ö
	...
	{
		path[0] &= 0xF0;
		return sendbyte;	// ÀÌµ¿À» ¾ÈÇß´Ù¸é
	}
	if (total == cur)
	{
		path[0] &= 0xF0;
		return sendbyte;	// ÀÌµ¿ÀÌ ³¡³µ´Ù¸é
	}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	//DevGamesX 18/11/2018 - BotSystem
	if (lpObj->Type == OBJ_USER || lpObj->IsBot >= 1)
	{
		if (lpObj->m_Change >= 0 && lpObj->IsBot == 0)
		{
...
BOOL gObjMoveGate(int aIndex, int gt) //Identical
{
	LPOBJ lpObj = &gObj[aIndex];
	int result;
	short x, y;
	...
	if (gt >= 58 && gt <= 61)
	{
		...
		{
			LogAdd("[DevilSquare] [%s][%s] Not Used DevilSquare Invitation In DevilSquare", gObj[aIndex].AccountID, gObj[aIndex].Name);
			return false;
		}
		else
		{
	...
	if (lpObj->m_bPShopOpen == 1)
	{
		...
		if (bMove == 0)
		{
			LogAdd("[PSHOP ITEM CLONNING] Move Failed : %d (%s %s)", result, lpObj->AccountID, lpObj->Name);
	...
	if (mapNumber == MAP_INDEX_ICARUS)
	{
		if (!(lpObj->pInventory[8].m_Type == ITEMGET(13, 3)
			...
			|| (lpObj->pInventory[7].m_Type >= ITEMGET(12, 100) && lpObj->pInventory[7].m_Type <= ITEMGET(12, 200))) //NEWWINGS
...
#if(GS_CASTLE==1)
						if (gObj[aIndex].MapNumber == MAP_INDEX_CASTLESIEGE) //HermeX Fix
						{
							if (g_CastleSiege.GetCastleState() == 7)
							{
								g_CastleSiege.NotifySelfCsJoinSide(aIndex);
...
#if(GS_CASTLE==1)
		if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE) //DiG Fix
		{
			if (g_CastleSiege.GetCastleState() == 7)
			{
				g_CastleSiege.NotifySelfCsJoinSide(aIndex);
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	if (!(lpObj->pInventory[target].m_Type < ITEMGET(12, 7)
		...
		|| (lpObj->pInventory[target].m_Type >= ITEMGET(12, 100) && lpObj->pInventory[target].m_Type <= ITEMGET(12, 200)))// NEWWINGS
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	if (!(lpObj->pInventory[target].m_Type < ITEMGET(12, 7)
		|| lpObj->pInventory[target].m_Type == ITEMGET(13, 30)
		|| (lpObj->pInventory[target].m_Type >= ITEMGET(12, 100) && lpObj->pInventory[target].m_Type <= ITEMGET(12, 200)))
		|| lpObj->pInventory[target].m_Type == ITEMGET(4, 7)
		|| lpObj->pInventory[target].m_Type == ITEMGET(4, 15))
	{
		return false;
	...
	{
		if (_r < Setting._Jewel2[lpObj->Custom->VipCount])
		{
			lpObj->pInventory[target].m_Level++;
		}
		else
	...
	{
		if (_r < Setting._Jewel1[lpObj->Custom->VipCount])
		{
			lpObj->pInventory[target].m_Level++;
		}
		else
	...
	LogAdd(lMsg.Get(559), lpObj->AccountID, lpObj->Name, lpObj->pInventory[source].m_Number, lpObj->pInventory[target].GetName(), lpObj->pInventory[target].m_Number, lpObj->pInventory[target].m_Level);
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	{
		if (lpObj->pInventory[target].m_Type >= ITEMGET(12, 3) && lpObj->pInventory[target].m_Type <= ITEMGET(12, 6) && lpObj->pInventory[target].m_Type >= ITEMGET(12, 100) && lpObj->pInventory[target].m_Type <= ITEMGET(12, 200))
		{
	...
	{
		if (_r < Setting._Jewel3[lpObj->Custom->VipCount])
		{
			lpObj->pInventory[target].m_Option3++;
		}
		else
...
BOOL gObjGuildWarCheck(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if (gObjGuildWarProc(lpGuild, lpTargetGuild, score) == 1)
	{
		...
		else
		{
			...
			gObjAddMsgSendDelay(lpObj, 4, lpObj->m_Index, 50000, 0);
		}
	}
...
BOOL gObjMonsterCall(int aIndex, int MonsterType, int x, int y)
{
	...
	if (result >= 0)
	{
		...
		LogAdd("[Summon Monster] [%s][%s] Try to Summon Monster - Succeed (SummonIndex:%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, result);
		return true;
	}
	return false;
}
...
{
	LogAdd("[Summon Monster] [%s][%s] Try to Kill Summoned Monster", gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	if (gObj[aIndex].m_RecallMon >= 0)
	{
		...
		{
			LogAdd("[Summon Monster] [%s][%s] Try to Kill Summoned Monster - Found Summoned Monster (SummonIndex:%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_RecallMon);
	...
	LogAdd("[Summon Monster] [%s][%s] Try to Kill Summoned Monster - Finished to Kill Summoned Monster (SummonIndex:%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, callmon);
}
...
int  gObjCheckAttackArea(int aIndex, int TarObjIndex) //Identical
{
...
void gObjSendUserStatistic(int aIndex, int startLevel, int endLevel)
{
	...
	sClassCount[WIZARD] = 0;
	sClassCount[KNIGHT] = 0;
	sClassCount[ELF] = 0;
	sClassCount[GLADIATOR] = 0;
	sClassCount[4] = 0;
	...
	MsgOutput(gObj[aIndex].m_Index, "[%d]-[%d] Total %d %s %d %s %d %s %d %s %d", startLevel, endLevel, sClassCount[4],
		lMsg.Get(1900), sClassCount[WIZARD],
		lMsg.Get(1901), sClassCount[KNIGHT],
		lMsg.Get(1902), sClassCount[ELF],
		lMsg.Get(1903), sClassCount[GLADIATOR]);
}
...
void gObjUseCircle(int aIndex, int pos)
{
	...
	LogAdd("[StatUp] [%s][%s] Use StatUpItem Level:%d Pos:%d serial:%d", gObj[aIndex].AccountID, gObj[aIndex].Name, level, pos, iSerial);
	...
	if (rand() % 100 < iSuccessRate)
	{
		...
		LogAdd("[StatUp] [%s][%s] Success [%d][%d] %d", gObj[aIndex].AccountID, gObj[aIndex].Name, level, incvalue, iSerial);
	}
	else
	{
		pResult.result |= 0x40;
		...
		DataSend(aIndex, (unsigned char *)&pResult, pResult.h.size);
		LogAdd("[StatUp] [%s][%s] Fail [%d] %d", gObj[aIndex].AccountID, gObj[aIndex].Name, level, iSerial);
		return;
	}
...
void MakeRewardSetItem(int aIndex, BYTE cDropX, BYTE cDropY, int iRewardType, int iMapnumber)
{
	...
	if (iRewardType == 1)
	{
		LogAdd("[¡Ú¡ÙReward][KUNDUN] [%s][%s] Set Item itemnum:[%d] skill:[%d] luck:[%d] option:[%d] SetOption:[%d]",
...
BOOL gObjGetRandomFreeLocation(int iMapNumber, BYTE &cX, BYTE & cY, int iRangeX, int iRangeY, int iLoopCount)
{
	...
	while (iLoopCount-- > 0)
	{
		...
		#else
				if( (attr & 4) != 4 ) //Perfect (Decompiled by HermeX)
				{
					if( (attr & 8) != 8)
					{
						return TRUE;
...
#if (GS_CASTLE==1)
BOOL gObjGetRandomFreeArea(int iMapNumber, BYTE &cX, BYTE &cY, int iSX, int iSY, int iDX, int iDY, int iLoopCount) //Identical (SirMaster Fix)
{
	if (iSX <= 0) iSX = 1;
	if (iSY <= 0) iSY = 1;
	if (iDX <= 0) iDX = 1;
