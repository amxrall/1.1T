...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	{
		auto it = this->m_vtChaosCastleOpenTime.begin();
		iMIN_HOUR = 24;
		iMIN_MINUTE = 60;
...
int  CChaosCastle::GetUserLevelToEnter(int iUserIndex)
{
	...
	for (int i = 0; i < MAX_CHAOSCASTLE_LEVEL; i++)
	{
		if (gObj[iUserIndex].Class == LORD ||
			gObj[iUserIndex].Class == GLADIATOR)
		{
...
void CChaosCastle::GiveUserDamage(int iUserIndex, int iDamage)
{
	...
	PHeadSetB((LPBYTE)&pResult, PROTO_ATTACK, sizeof(pResult));
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		if (gObj[iUserIndex].Class == LORD || gObj[iUserIndex].Class == GLADIATOR)
		{
...
BOOL CChaosCastle::ObjSetPosition(int iIndex, int iX, int iY)
{
	...
	PHeadSetB((LPBYTE)&pMove2, PROTO_SETPOS, sizeof(pMove2));
...
void CChaosCastle::GiveWinnerItem(int iChaosCastleIndex, int iWinnerIndex)
{
	...
	switch (gObj[iWinnerIndex].MapNumber)
	{
	case 18:
		if (ChaosEvent._Resets[0] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", ChaosEvent._Resets[0], gObj[iWinnerIndex].Name);
		}

		ChaosEvent.Item(iWinnerIndex, 0);
		break;
	case 19:
		if (ChaosEvent._Resets[1] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", ChaosEvent._Resets[1], gObj[iWinnerIndex].Name);
		}

		ChaosEvent.Item(iWinnerIndex, 1);
		break;
	case 20:
		if (ChaosEvent._Resets[2] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", ChaosEvent._Resets[2], gObj[iWinnerIndex].Name);
		}

		ChaosEvent.Item(iWinnerIndex, 2);
		break;
	case 21:
		if (ChaosEvent._Resets[3] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", ChaosEvent._Resets[3], gObj[iWinnerIndex].Name);
		}

		ChaosEvent.Item(iWinnerIndex, 3);
		break;
	case 22:
		if (ChaosEvent._Resets[4] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", ChaosEvent._Resets[4], gObj[iWinnerIndex].Name);
		}

		ChaosEvent.Item(iWinnerIndex, 4);
		break;
	case 23:
		if (ChaosEvent._Resets[5] > 0)
		{
			Manager.ExecFormat("UPDATE Character SET Resets = Resets + %d WHERE Name = '%s'", ChaosEvent._Resets[5], gObj[iWinnerIndex].Name);
		}

		ChaosEvent.Item(iWinnerIndex, 5);
		break;
	default:
		break;
	}
