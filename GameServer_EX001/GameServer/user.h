...
#define OBJ_MAXMONSTER 7800
#else
#define OBJMAX 6000
#define OBJMAXUSER 1500
#define OBJ_MAXMONSTER 3500
...
#define NEWWINGS(x) ((x < ITEMGET(12, 100)) ? FALSE : (x > ITEMGET(12, 200)) ? FALSE : TRUE)
#define NEWPETS(x)	((x < ITEMGET(13,100)) ? FALSE : (x > ITEMGET(13,150)) ? FALSE : TRUE)
...
struct OBJECTSTRUCT
{
	...
	char m_Drink;
	int m_SkillDefense;
	int m_SkillDefenseTime;
	int m_SkillAttack;
	int m_SkillAttackTime;
	int m_SkillAttack2;
	...
	int SkillAddCriticalDamageTime;
	int SkillAddCriticalDamage;
...
public:
	CUSTOMSTRUCT() :
		Bless(0), Soul(0), Life(0), Chaos(0), Creation(0), Guardian(0), m_CSCoin(0), m_Gold(0),m_Cash(0),ResetCount(0), MasterCount(0), VipCount(0), Vault(0), Hours(0), Jewels{ 0, 0, 0, 0, 0, 0 }, Killer(0), Hero(0), Bonus(0), Coin{ 0, 0 ,0, 0, 0}, Waiting{ false, false }, Move(false), InEvent(false), InDuel(false), Dead(false), Hidden(false), Start(false), Delay{ 0, 0, 0 }, Time(0)
	{
	}
	int		m_CSCoin;
	int		m_Gold;
